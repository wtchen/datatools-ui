...
export function filterAlertsByCategory (alerts, filter) {
  const now = moment()
  switch (filter) {
    case 'ALL':
      return alerts
    ...
    case 'FUTURE':
      return alerts.filter(alert => moment(alert.start).isAfter(now))
    case 'ARCHIVED':
      return alerts.filter(alert => moment(alert.end).isBefore(now))
    case 'DRAFT':
      return alerts.filter(alert => !alert.published)
    default:
      return alerts
  }
}
