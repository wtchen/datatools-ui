...
export default class AffectedEntity extends Component {
  ...
  renderHeader (entity, feeds, onDeleteEntityClick) {
    return (
      <Row
      ...
      >
        ...
        <Col xs={2}>
          <Button
            ...
            style={{marginTop: '5px'}}
            onClick={() => onDeleteEntityClick(entity)}>
            <Icon type='remove' />
          </Button>
        </Col>
      </Row>
  ...
  render () {
    ...
    return (
      <ListGroupItem
        bsStyle={(entity && entity.type === 'AGENCY') || (entity && entity.type === 'MODE') ? 'warning' : 'default'}
        header={this.renderHeader(entity, feeds, onDeleteEntityClick)}
      >
