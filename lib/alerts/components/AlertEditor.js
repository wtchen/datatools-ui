...
import {
  ...
  onAlertEditorMount as onAlertEditorMountAction,
  saveAlert as saveAlertAction,
  setActiveAlert as setActiveAlertAction
} from '../actions/alerts'
import {
  setActiveProperty as setActivePropertyAction,
  setActivePublished as setActivePublishedAction,
...
import type {ManagerUserState, RouterLocation} from '../../types/reducers'

type Props = {
  activeFeeds: Array<Feed>,
  addActiveEntity: typeof addActiveEntityAction,
  alert: Alert,
  createAlert: typeof createAlertAction,
  deleteActiveEntity: typeof deleteActiveEntityAction,
  deleteAlert: typeof deleteAlertAction,
  editableFeeds: Array<Feed>,
  location: RouterLocation,
  onAlertEditorMount: typeof onAlertEditorMountAction,
  permissionFilter: string,
  project: Project,
  publishableFeeds: Array<Feed>,
  saveAlert: typeof saveAlertAction,
  setActiveAlert: typeof setActiveAlertAction,
  setActiveProperty: typeof setActivePropertyAction,
  setActivePublished: typeof setActivePublishedAction,
...
export default class AlertEditor extends Component<Props> {
  ...
  validateAndSave = () => {
    ...
    if (!momentEnd.isValid() || !momentStart.isValid()) {
  ...
  _deleteAlert = () => this.props.deleteActiveEntity(this.props.alert)
  ...
  _onRouteClick = (stop: GtfsRoute, agency: any, newEntityId: ?number) => {
    this.props.addActiveEntity('ROUTE', stop, agency, newEntityId)
  }

  _onStopClick = (stop: GtfsStop, agency: any, newEntityId: ?number) => {
  ...
  render () {
    ...
    const newEntityId = alert.affectedEntities && alert.affectedEntities.length
      ? 1 + alert.affectedEntities
        .map(e => e.id)
        .reduce((initial, current) => initial > current ? initial : current)
      : 1
    return (
      <ManagerPage
        ref='page'
