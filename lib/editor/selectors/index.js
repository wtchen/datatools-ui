...
export const getActivePatternLocations = createSelector(
  ...
  (pattern, locations, locationGroups) => {
    if (!pattern || (!locations && !locationGroups) || !pattern.patternLocations) return null
    const patternLocations = pattern.patternLocations.map(pl => {
      return (locations || []).find(l => l.location_id === pl.locationId) || (locationGroups || []).find(lg => lg.location_group_id === pl.locationId)
    })
    return patternLocations.filter(pl => !!pl)
  }
)
