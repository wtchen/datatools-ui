...
const getStopAreas = (state: AppState) => state.editor.data.tables.stop_areas
...
export const getActivePatternStopAreas = createSelector(
  [
    getActivePattern,
    getStopAreas
  ],
  (pattern, stopAreas) => {
    if (!pattern || !stopAreas || !pattern.patternStopAreas) return null
    const patternStopAreas = pattern.patternStopAreas.map(psa => {
      return stopAreas.find(sa => sa.area_id === psa.areaId)
    })
