...
export const getLocations = (state: AppState) => state.editor.data.tables.locations
const getLocationGroups = (state: AppState) => state.editor.data.tables.location_groups
...
export const getActivePatternLocations = createSelector(
  [
    getActivePattern,
    getLocations
  ],
  (pattern, locations) => {
    if (!pattern || !locations || !pattern.patternLocations) return null
    const patternLocations = pattern.patternLocations.map(pl => {
      return locations.find(l => l.location_id === pl.locationId)
    })
    return patternLocations
  }
)
export const getActivePatternLocationGroups = createSelector(
  [
    getActivePattern,
    getLocationGroups
  ],
  (pattern, locationGroups) => {
    if (!pattern || !locationGroups || !pattern.patternLocationGroups) return null
    const patternLocationGroups = pattern.patternLocationGroups.map(pl => {
      return locationGroups.find(lg => lg.location_group_id === pl.locationGroupId)
    })
