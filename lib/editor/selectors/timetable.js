...
const getLocations = (state: AppState) => getTableById(state.editor.data.tables, 'location')
...
export const getTimetableColumns = createSelector(
  [ getActivePattern, getStops, getLocations ],
  (pattern: ?Pattern, stops: Array<GtfsStop>, locations: Array<GtfsLocation>): Array<TimetableColumn> => {
    const columns = [
      {
        name: 'Block ID',
        width: 60,
    ...
    if (pattern && pattern.patternStops) {
      if (!pattern.useFrequency) {
        ...
        patternHalts.map((ps, index) => {
          const isLocation = !patternHaltIsStop(ps)
          let stop = null
          // $FlowFixMe flow doesn't understand hasOwnProperty
          ...
          // $FlowFixMe FLEX TODO: improve this to avoid quadruple ternary
          const stopName = stop ? stop.stop_name : ps.stopId ? ps.stopId : (ps.locationId ? ps.locationId : ps.areaId)
          const stopId = stop && stop.stop_code
            ? stop.stop_code
            : stop && stop.stop_id
              ? stop.stop_id
              : stop && stop.area_id ? stop.area_id : stop && stop.location_id ? stop.location_id : ''
          const abbreviatedStopName = stop
            ? getAbbreviatedStopName(stop)
          // $FlowFixMe FLEX TODO: improve this to avoid triple ternary
            : ps.stopId ? ps.stopId : (ps.locationId ? ps.locationId : ps.areaId)
          const TIME_WIDTH = 80
          columns.push({
            name: abbreviatedStopName,
            // $FlowFixMe FLEX TODO: clean this file up
            title: `${stopName} (${stopId})`,
            width: TIME_WIDTH,
            key: `stopTimes.${index}.${isLocation ? 'startPickupDropoffWindow' : 'arrivalTime'}`,
            colSpan: '2',
            type: 'ARRIVAL_TIME',
            placeholder: 'HH:MM:SS'
          })
          columns.push({
            key: `stopTimes.${index}.${isLocation ? 'endPickupDropoffWindow' : 'departureTime'}`,
            width: TIME_WIDTH,
            type: 'DEPARTURE_TIME',
            placeholder: 'HH:MM:SS'
          })
