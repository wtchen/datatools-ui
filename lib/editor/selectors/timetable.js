      ...
      if (!pattern.useFrequency) {
        ...
        patternHalts.map((ps, index) => {
          ...
          const stopId = stop && stop.stop_code
            ...
            : stop && stop.stop_id
              ? stop.stop_id
              : stop && stop.location_group_id ? stop.location_group_id : stop && stop.location_id ? stop.location_id : ''
          const abbreviatedStopName = stop
            ? getAbbreviatedStopName(stop)
          // $FlowFixMe FLEX TODO: improve this to avoid triple ternary
            : ps.stopId ? ps.stopId : (ps.locationId ? ps.locationId : ps.locationGroupId)
          const TIME_WIDTH = 80
          columns.push({
