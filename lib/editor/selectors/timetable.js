  ...
  (pattern: ?Pattern, stops: Array<GtfsStop>, locations: Array<GtfsLocation>): Array<TimetableColumn> => {
    ...
    if (pattern && pattern.patternStops) {
      if (!pattern.useFrequency) {
        const patternHalts = mergePatternHalts(pattern.patternStops, pattern.patternLocations)
        patternHalts.map((ps, index) => {
          const isLocation = ps.hasOwnProperty('locationId')
          let stop = null
          ...
          // $FlowFixMe FLEX TODO: improve this to avoid double ternary
          const stopName = stop ? stop.stop_name : ps.stopId ? ps.stopId : ps.locationId
          const stopId = stop && stop.stop_code
            ? stop.stop_code
            : stop && stop.stop_id
              ? stop.stop_id
              : stop && stop.location_id ? stop.location_id : ''
          const abbreviatedStopName = stop
            ? getAbbreviatedStopName(stop)
          // $FlowFixMe FLEX TODO: improve this to avoid double ternary
            : ps.stopId ? ps.stopId : ps.locationId
          const TIME_WIDTH = 80
          columns.push({
