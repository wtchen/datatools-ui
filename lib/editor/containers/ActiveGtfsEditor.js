...
import {getControlPoints, getPatternCoordinates, getValidationErrors} from '../selectors'
...
const mapStateToProps = (state, ownProps) => {
  ...
  // const patternCoordinates = getPatternCoordinates(state)
  // console.log('pattern coordinates', patternCoordinates)
  const temporaryPatternCoordinates = state.editor.editSettings.patternCoordinates
  ...
  const feedInfo = getTableById(tables, 'feedinfo')
...
const mapDispatchToProps = (dispatch, ownProps) => {
  ...
  return {
    ...
    // dispatch(handleEditorMount(initialProps, ownProps))
    ...
    onComponentUpdate: (prevProps, newProps) => {
      ...
      if (prevProps.activeEntityId !== ENTITY.NEW_ID &&
        ...
        // FIXME: handle infinite loop case when, for example, entity ID is not found
