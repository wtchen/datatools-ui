...
const mapStateToProps = (state: AppState, ownProps: GtfsEditor & {routeParams: any, location: any}) => {
  const {
    feedSourceId,
    activeComponent,
    subComponent,
...
const mapDispatchToProps = (dispatch: dispatchFn, ownProps: GtfsEditor & {routeParams: any, location: any}) => {
  const {
    feedSourceId,
    activeComponent,
    subComponent,
  ...
  return {
    ...
    onComponentMount: (initialProps, forceFetch) => {
      ...
      if (!feedSource || feedSourceId !== feedSource.id) {
        ...
        dispatch(fetchFeedSourceAndProject(feedSourceId))
          ...
          .then(() => dispatch(setActiveGtfsEntity(feedSourceId, activeComponent, activeEntityId, subComponent, subEntityId, subSubComponent, activeSubSubEntity, forceFetch)))
      } else {
    ...
    updateEditSetting: (setting, value) => dispatch(updateEditSetting(setting, value)),
    ...
    removeStopFromPattern: (pattern, stop, index) => dispatch(removeStopFromPattern(pattern, stop, index)),
    ...
    updatePatternStops: (pattern, patternStops) => dispatch(updatePatternStops(pattern, patternStops)),
