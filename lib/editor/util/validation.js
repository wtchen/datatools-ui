...
export function validate (field, value, entities, entity, tableData) {
  ...
  switch (inputType) {
    case 'GTFS_ID':
      ...
      const isNotUnique = value && (indices.length > 1 || (indices.length && entities[indices[0]].id !== entity.id))
    ...
    case 'TEXT':
      if (name === 'route_short_name' && !value && entity.route_long_name) {
        return false
      } else if (name === 'route_long_name' && !value && entity.route_short_name) {
        return false
      } else {
    ...
    case 'NUMBER':
      ...
      if (isRequiredButEmpty || isNotANumber) {
        ...
        return {field: name, invalid: isRequiredButEmpty || isNotANumber, reason}
      } else {
        return false
      }
    case 'DAY_OF_WEEK_BOOLEAN':
      ...
      if (!hasService && name === 'monday') { // only add validation issue for one day of week (monday)
        reason = 'Calendar must have service for at least one day'
        return {field: name, invalid: isRequiredButEmpty, reason}
      }
      return false
    case 'DROPDOWN':
      if (isRequiredButEmpty && field.options && field.options.findIndex(o => o.value === '') === -1) {
        return {field: name, invalid: isRequiredButEmpty, reason}
      } else {
        return false
      }
    case 'GTFS_AGENCY':
      if (name === 'agency_id' && tableData.agency && tableData.agency.length > 1) {
    ...
    case 'EXCEPTION_DATE': // a date cannot be selected more than once (for all exceptions)
      ...
      for (let i = 0; i < exceptions.length; i++) {
        exceptions[i].dates && exceptions[i].dates.map(d => {
      ...
      for (let i = 0; i < value.length; i++) {
        if (dateMap[value[i]] && dateMap[value[i]].length > 1) {
          reason = `Date (${value[i]}) cannot appear more than once for all exceptions`
