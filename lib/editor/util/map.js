...
export const stopIsOutOfBounds = (stop: GtfsStop | GtfsLocation, bounds: any) => {
  if (stop.location_id) {
    const {location_shapes: locationShapes} = stop

    const PADDING = 0.15
    return locationShapes.some(locationShapePt =>
      locationShapePt.geometry_pt_lat > bounds.getNorth() + PADDING ||
      locationShapePt.geometry_pt_lat < bounds.getSouth() - PADDING ||
      locationShapePt.geometry_pt_lon > bounds.getEast() + PADDING ||
      locationShapePt.geometry_pt_lon < bounds.getWest() - PADDING
    )
  // Check for stop_id to appease flow
...
export function resequenceStops (
  stop: PatternStop | PatternLocation | PatternLocationGroup | StopTime,
  index: number
): any {
  return {
    ...stop,
...
export function stopToPatternStop (
...
): PatternStop | PatternLocation | PatternLocationGroup {
  if (stop.hasOwnProperty('location_group_id')) {
    return {
      continuousDropOff: 1,
      continuousPickup: 1,
      ...
      flexDefaultZoneTime: 0,
      id: generateUID(),
      // $FlowFixMe Flow doesn't appreciate our type check above
      locationGroupId: stop.location_group_id,
      meanDurationFactor: 0,
      meanDurationOffset: 0,
      pickupBookingRuleId: null,
      pickupType: 0,
      safeDurationFactor: 0,
      safeDurationOffset: 0,
      stopSequence,
      timepoint: null
    }
  }
  ...
  // If it is not a location or a location group, treat the patternStop as a stop
  if (!stop.stop_id) {
