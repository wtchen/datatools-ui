...
export function isSegmentActive (activeIndex, index) {
...
export function assignDistancesToPatternStops (patternStops, shapePoints) {
  const patternStopsClone = clone(patternStops)
  const filteredShapePoints = shapePoints.filter(sp => sp.pointType === 2)
 ...
 * value is 0.
 */
export function stopToStopTime (stop: GtfsStop, stopSequence: number = 0): StopTime {
  if (!stop.stop_id) {
