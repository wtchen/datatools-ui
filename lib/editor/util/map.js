  ...
  if (editType === 'update') {
    ...
    if (index === 0) {
      ...
      if (controlPoints[index].stopId) {
        ...
        updatedControlPoints = controlPoints.slice()
 ...
 */
export function resequenceStops (stop: PatternStop | StopTime, index: number): PatternStop {
  return {
    ...stop,
    stopSequence: index
  }
...
export function getPatternDistance (
  ...
  if (shape) {
    ...
    if (controlPoints.length > 0) {
      line = lineString(controlPoints.map(cp => cp.point.geometry.coordinates))
    }
  }
  return line ? lineDistance(line, unit) : 0
}

export function projectStopOntoLine (stop: GtfsStop, line: any): any {
  const stopPoint = stopToPoint(stop)
