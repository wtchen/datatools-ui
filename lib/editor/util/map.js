...
function cloneControlPointWithDistanceOffset (
  ...
  clonedControlPoint.shapeDistTraveled = controlPoint.shapeDistTraveled + offsetDistance
  clonedControlPoint.shapePtSequence = controlPoint.shapePtSequence + offsetIndex
...
export function getFeedBounds (
  ...
  return feedSource && feedSource.latestValidation && feedSource.latestValidation.bounds && !isNaN(feedSource.latestValidation.bounds.north)
...
export async function recalculateShape ({
  ...
  if (editType === 'update') {
    ...
    if (index === 0) {
      ...
      updatedControlPoints = controlPoints.slice(sliceIndex).map(controlPoint =>
      ...
      return {coordinates: updatedPatternCoordinates, dragId, updatedControlPoints}
      ...
      if (controlPoints[index].stopId) {
        ...
        const {previousStopIdx, followingStopIdx} = findSurroundingStopIndexes(index, controlPoints)
        ...
        if (followingStopIdx === -1) {
          ...
          const newCoords = updatedPatternCoordinates.slice(0, previousStopIdx + 1)
          return {
  ...
  if (!newSegment || !newSegment.coordinates) {
    ...
    console.warn(`Failed to route between points for control point ${index}`, lineString(pointsToRoute))
    return {
      coordinates: null,
      dragId,
  ...
  console.log('routed segment', newSegment)
  let indexOffset
  ...
  if (editType === 'update') {
    ...
    if (snapControlPointToNewSegment) {
      const {snapPoint, distTraveled, index: snapIndex, beforeSlice, afterSlice} = getControlPointSnap(
        controlPointGeoJson,
        ensureValidCoords(newSegment.coordinates)
      )
      // Update surrounding pattern coordinate segments
      if (index > 0) {
      ...
      const previousSequence = previousControlPoint ? previousControlPoint.shapePtSequence : 0
      const previousDistance = previousControlPoint ? previousControlPoint.distance : 0
      const followingSequence = followingControlPoint ? followingControlPoint.shapePtSequence : controlPoints[index].shapePtSequence
      indexOffset = newSegment.coordinates.length - (followingSequence - previousSequence)
    ...
    updatedPatternCoordinates.splice(segmentSpliceIndex, 2, newSegment.coordinates)
  }
  ...
  controlPoints.slice(index + 1).forEach(controlPoint =>
    updatedControlPoints.push(cloneControlPointWithDistanceOffset(controlPoint, offset, indexOffset))
  )
...
export function shapePointsToSimpleCoordinates (shapePoints) {
  return shapePoints
    ? shapePoints.map(shapePoint => ([shapePoint.shapePtLon, shapePoint.shapePtLat]))
    : []
}
 ...
 */
export function assignDistancesToPatternStops (patternStops: Array<PatternStop>, shapePoints: Array<ShapePoint>): any {
  const patternStopsClone = clone(patternStops)
  // Filter out only the shape points that are associated with a stop.
  const filteredShapePoints = shapePoints.filter(sp => sp.pointType === POINT_TYPE.STOP)
 ...
 * segments.
 */
export function constructShapePoints (controlPoints: Array<ControlPoint>, patternSegments: Array<any>): any {
  const shapePoints = []
  ...
  for (var i = 0; i < controlPoints.length; i++) {
    ...
    if (segment) {
      ...
      shapePoints.push(...coordinatesToShapePoints(segment, lastDistance, shapePoints.length))
    }
  }
...
function coordinatesToShapePoints (coordinates, initialDistance = 0, initialSequence = 0) {
  let previousPoint
...
export function getLineSlices (lineSegment, pointOnLine) {
  const beginPoint = point(lineSegment.geometry.coordinates[0])
  const endPoint = point(lineSegment.geometry.coordinates[lineSegment.geometry.coordinates.length - 1])
...
function getOffsetDistance (editType, index, newSegment, controlPoints) {
  let offsetDistance
  ...
  if (editType === 'update' && index === 0) {
    ...
    offsetDistance = segmentDistance - (followingControlPoint.distance - previousControlPoint.distance)
  }
 ...
 * value is 0.
 */
export function stopToPatternStop (stop: GtfsStop, stopSequence: number = 0): PatternStop {
  if (!stop.stop_id) {
