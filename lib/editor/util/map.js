...
export async function recalculatePatternCoordinates (
  ...
  // validate arguments
  if (editType === 'insert' && !newPoint) {
    throw new Error('newPoint argument is required with editType="insert"')
  }

  if (index === controlPoints.length - 1 && editType === 'update') {
    throw new Error('invalid arguments for recalculatePatternCoordinates')
  }
  ...
  function lineSliceAlongToCoords (startDistance, endDistance) {
    return lineSliceAlong(lineString(patternShape), startDistance, endDistance)
  ...
  if (editType === 'insert') {
    const newPointCoordinates = ll.toCoordinates(newPoint)
    if (index === 0) {
    ...
    points.push(currentControlPoint.point.geometry.coordinates)
