...
export function getRouteStatusText (statusCode: number) {
  const table = getEditorTable('route')
  if (table) {
    const statusField = table.fields.find(field => field.name === 'status')
    if (statusField && statusField.options) {
      const option = statusField.options.find(option => option.value === statusCode)
      if (option) {
        return option.text
      }
    }
...
export function routeStatusTextToValue (statusText: string) {
  const table = getEditorTable('route')
  if (table) {
    const statusField = table.fields.find(field => field.name === 'status')
    if (statusField && statusField.options) {
      const option = statusField.options.find(option => option.text === statusText)
      if (option) {
        return option.value
      }
    }
