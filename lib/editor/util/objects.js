...
export const getMapFromGtfsStrategy = (component: string) => {
  switch (component) {
    case 'location':
      return (entity: any) => {
        // Delete the UI-only flag for rendering saved shapes
        if (entity.location_shapes) {
          entity.location_shapes.forEach(locationShape => {
            if (locationShape.hasOwnProperty('fromSaved')) delete locationShape.fromSaved
          })
        }
        return snakeCaseKeys(entity)
      }
    default:
