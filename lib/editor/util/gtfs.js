...
export const COMPONENT_LIST = [
  ...
  { id: 'location', tableName: 'locations' },
  { id: 'locationgroup', tableName: 'location_groups' },
  ...
  { id: 'bookingrule', tableName: 'booking_rules' }
]
...
export const generateProps = (component: string, editorState: any): any => {
  ...
  switch (component) {
    case 'location':
      const locationId = generateUID()
      // TODO: Center on the new location
      return {
...
export function getEntityBounds (entity: any, stops?: Array<GtfsStop>, locations?: Array<GtfsLocation>, offset: number = 0.005): ?latLngBounds {
  function findLocationCoodinates (coordinateArray) {
    const lats = []
    const lons = []

    coordinateArray.forEach(point => {
      // $FlowFixMe flow doesn't understand this but it works
      lats.push(point.geometry_pt_lat || point.stop_lat)
      // $FlowFixMe flow doesn't understand this but it works
      lons.push(point.geometry_pt_lon || point.stop_lon)
    })

    return [
      [Math.max.apply(null, lats) + offset, Math.min.apply(null, lons) - offset],
      [Math.min.apply(null, lats) - offset, Math.max.apply(null, lons) + offset]
    ]
  }
  ...
  if (entity.constructor === Array) {
  ...
  } else if (Array.isArray(entity.location_shapes)) {
    // location
    const { location_shapes: locationShapes } = entity

    return latLngBounds(findLocationCoodinates(locationShapes))
    ...
    if (route.tripPatterns) {
      route.tripPatterns.map(pattern => {
        ...
        let flexRouteCoordinates = []

        // check for flex route locations

        if ((locations && pattern.patternLocations && pattern.patternLocations.length > 0) || (stops && pattern.patternStops && pattern.patternStops.length > 0)) {
          // finds matching locations and coordinates in state + concats all locations into one array of coordinate objects

          const locationCoordinates = pattern.patternLocations
            .map(pl => locations ? locations.find(l => l.location_id === pl.locationId) : null)
          // $FlowFixMe flow doesn't understand flatMap but it works
            .flatMap(pl => pl ? pl.location_shapes : null)

          // add all flex coorinates to array

          flexRouteCoordinates = [
            ...flexRouteCoordinates,
            ...locationCoordinates || [],
            ...stops || []
          ]
        ...
        // find corners of all location and stop coordinates

        const resolvedFlexCoordinates = flexRouteCoordinates.length > 0 ? findLocationCoodinates(flexRouteCoordinates) : []

        coordinates = [
          ...coordinates,
          ...resolvedFlexCoordinates
        ]

        // TODO: support location group boundaries -- need to support location group boundaries first
      }
      )
    }
...
export function getEntityName (entity: any): string {
  ...
  if (typeof entity.route_id !== 'undefined') {
    ...
    nameKey = 'agency_name'
    // Location Groups have a stop id, but shouldn't be named as a location group
  } else if (typeof entity.stop_id !== 'undefined' && typeof entity.location_group_id === 'undefined') {
    nameKey = 'stop_name'
    // Location Groups have a location id, but shouldn't be named as a location group
  } else if (typeof entity.location_id !== 'undefined' && typeof entity.location_group_id === 'undefined') {
    nameKey = 'location_name'
  } else if (typeof entity.service_id !== 'undefined') {
    nameKey = 'description'
  } else if (typeof entity.fare_id !== 'undefined') {
    nameKey = 'fare_id'
  } else if (typeof entity.booking_rule_id !== 'undefined') {
    nameKey = 'booking_rule_id'
  } else if (typeof entity.location_group_id !== 'undefined') {
    nameKey = 'location_group_name'
  ...
  switch (nameKey) {
    ...
    case 'location_name':
      const location: GtfsLocation = ((entity: any): GtfsLocation)
      return location.stop_name && location.location_id
        ? `${location.stop_name} (${location.location_id})`
        : location.location_id
    case 'location_group_name':
      const locationGroup: GtfsLocation = ((entity: any): GtfsLocation)
      if (locationGroup.location_group_id) {
        if (locationGroup.location_group_name) {
          return `${locationGroup.location_group_name} (${locationGroup.location_group_id})`
        }
        return locationGroup.location_group_id
      }
      return NO_NAME
...
export function getAbbreviatedStopName (stop: GtfsStop | GtfsLocation, maxCharactersPerWord: number = 10): string {
