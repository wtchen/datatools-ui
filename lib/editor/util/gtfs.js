  ...
  if (entity.constructor === Array) {
    ...
    if (route.tripPatterns) {
      route.tripPatterns.map(pattern => {
        ...
        if (locations && pattern.patternLocations && pattern.patternLocations.length > 0) {
          // finds matching locations and coordinates in state + concats all locations into one array of coordinate objects

          const locationCoordinates = pattern.patternLocations.map(pl => locations.find(l => l.location_id === pl.locationId)).map(pl => pl.location_shapes).flat(1)
        ...
        // check for flex trip pattern stops, and add to flex coordinates array
