...
export const GTFS_PLUS_EDITOR_LOCK = 'gtfs-lock-editor'
...
export function fetchBaseGtfsWithoutLock ({
  feedSourceId,
  namespace
}: {
  feedSourceId?: string,
...
/**
 * This variant also locks the feed (to accommodate existing code).
 */
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    return dispatch(lockEditorFeedSourceIfNeeded(feedSourceId, GTFS_EDITOR_LOCK))
      .then(lockSuccess => {
        ...
        return dispatch(fetchBaseGtfsWithoutLock({ feedSourceId, namespace }))
      })
  }
}
