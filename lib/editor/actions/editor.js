...
const createGtfsEntity = createAction('CREATE_GTFS_ENTITY')
...
export function cloneGtfsEntity (feedSourceId, component, entityId, save = false) {
...
export function newGtfsEntity (feedSourceId, component, props, save = false, refetch = true) {
  return function (dispatch, getState) {
    if (!props) {
    ...
    if (save) {
      ...
      if (!feedSourceId) {
        ...
        feedSourceId = getState().editor.data.active.feedSourceId
      }
      dispatch(createGtfsEntity({feedSourceId, component, props, save, refetch}))
