...
import type { ControlPoint, Pattern, PatternHalt } from '../../types'
import type {dispatchFn, getStateFn} from '../../types/reducers'
import { mergePatternHaltsOfPattern } from '../../gtfs/util'
import { patternHaltIsLocation, patternHaltIsStop, patternHaltIsStopArea } from '../util/location'
...
export function updatePatternStops (
  pattern: Pattern,
  patternHalts: Array<PatternHalt>
) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    const sortedPatternHalts = patternHalts.map(resequenceStops)
    const { locations, stopAreas, stops } = sortedPatternHalts.reduce(
      (acc, cur) => {
        if (patternHaltIsLocation(cur)) {
          acc.locations.push(cur)
        }
        if (patternHaltIsStopArea(cur)) {
          acc.stopAreas.push(cur)
        }
        if (patternHaltIsStop(cur)) {
          acc.stops.push(cur)
        }
        return acc
      },
      { locations: [], stopAreas: [], stops: [] }
    )
    dispatch(
      updateActiveGtfsEntity(
        {
          ...
          entity: pattern,
          props: { patternLocations: locations, patternStopAreas: stopAreas, patternStops: stops }
        }
      )
    )
  }
...
export function saveTripPattern (feedId: ?string, tripPattern: Pattern) {
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    const patternHalts = mergePatternHaltsOfPattern(tripPattern)
      .map(resequenceStops)
    tripPattern.patternStops = patternHalts.filter(patternHaltIsStop)
    ...
    // $FlowFixMe FLEX TODO: this type check looks good, but flow is complaining? why?
    tripPattern.patternStopAreas = patternHalts.filter(patternHaltIsStopArea)
