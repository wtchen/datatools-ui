...
import type { PatternStop, ControlPoint, Pattern, PatternLocation, PatternLocationGroup } from '../../types'
...
export function updatePatternStops (
  pattern: Pattern,
  patternHalts: Array<PatternStop | PatternLocation | PatternLocationGroup>
) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    const sortedPatternHalts = patternHalts.map(resequenceStops)
    const {stops, locationGroups, locations} = sortedPatternHalts.reduce(
      (acc, cur) => {
        if (patternHaltIsLocation(cur)) {
          acc.locations.push(cur)
        }
        if (patternHaltIsLocationGroup(cur)) {
          acc.locationGroups.push(cur)
        }
      ...
      { stops: [], locations: [], locationGroups: [] }
    )
    dispatch(
