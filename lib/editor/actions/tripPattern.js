...
import type {ControlPoint, Pattern, PatternLocation, PatternStop} from '../../types'
...
export function updatePatternStops (
  pattern: Pattern,
  patternHalts: Array<PatternStop | PatternLocation>
) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    const sortedPatternHalts = patternHalts.map(resequenceStops)
    const {stops, locations} = sortedPatternHalts.reduce(
      (acc, cur) => {
        if (cur.hasOwnProperty('locationId')) {
          acc.locations.push(cur)
        }
        if (cur.hasOwnProperty('stopId')) {
          acc.stops.push(cur)
        }
        return acc
      },
      ...
      updateActiveGtfsEntity(
        {
          ...
          entity: pattern,
          props: { patternStops: stops, patternLocations: locations }
        }
      )
    )
  }
...
export function saveTripPattern (feedId: ?string, tripPattern: Pattern) {
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    const patternHalts = mergePatternHalts(tripPattern.patternStops, tripPattern.patternLocations)
      .map(resequenceStops)
    tripPattern.patternStops = patternHalts.filter(ph => ph.hasOwnProperty('stopId'))
    // $FlowFixMe FLEX TODO: this type check looks good, but flow is complaining? why?
    tripPattern.patternLocations = patternHalts.filter(ph => ph.hasOwnProperty('locationId'))
