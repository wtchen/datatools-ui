...
import type {Entity, Feed, LocationShape} from '../../types'
...
export const updatingLocationShape = createAction(
  ...
  (payload: Array<LocationShape>) => payload
)
export const settingInitialLocationIds = createAction(
  'SETTING_INITIAL_LOCATION_IDS',
  (payload: {initialIds: Array<string>}) => payload
)
...
/**
 * Updates the inital location shape IDs for the active entity.
 * Used to render saved polygons separately from those created
 * by React-leaflet-draw's EditControl polygon editor.
 */
export function setInitialLocationShapeIds (feedSourceId: string, component: string, entity: Entity | { id: number }) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    const locationShapes = activeEntity.location_shapes

    if (locationShapes) {
      const initialLocationIds = locationShapes.map(shape => shape.geometry_id)
      dispatch(settingInitialLocationIds(initialLocationIds))
    }
  }
}
...
export function setActiveGtfsEntity (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    // TODO: fix this hack to persist the initialLocationIds when loading active entities
    // This will make the initial_location_shape_ids state persist unless overridden by
    // setInitialLocationShapeIds
    if (activeEntity) activeEntity.initial_location_shape_ids = data.active.entity ? data.active.entity.initial_location_shape_ids : []
