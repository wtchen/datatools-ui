...
import {fetchGraphQL, createVoidPayloadAction, secureFetch} from '../../common/actions'
...
import type {Entity, Feed, GeometryType, LocationShape} from '../../types'
import type {dispatchFn, getStateFn, AppState} from '../../types/reducers'
import {getEntityGraphQLRoot} from '../../gtfs/util'

import {lockEditorFeedSource, receiveBaseGtfs, removeEditorLock, newGtfsEntity, fetchBaseGtfs} from './editor'
...
export const updatingLocationShape = createAction(
  'UPDATING_LOCATION_SHAPE',
  (payload: {
...
/**
 * Fetches a list of routes from the backend for the editor.
 * Requires a lock to function.
 */
export function fetchRouteEntities (feedSource: Feed) {
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    const { lock } = getState().editor.data
    let lockCreated = false
    // We need to get a lock if we do not already have one
    if (!lock.sessionId) {
      await dispatch(lockEditorFeedSource(feedSource.id))
      // Save that lock was created so it can be removed when we are done.
      lockCreated = true
    }

    const namespace = getEditorNamespace(
      getState().editor.data.lock.feedId,
      getState()
    )
    if (!namespace) {
    ...
    const graphQLRoot = getEntityGraphQLRoot('route')
    if (!graphQLRoot) {
      console.warn(`No graphql table for route.`)
    }

    const query = `
      query entityQuery($namespace: String) {
    ...
    const data = await dispatch(fetchGraphQL({ query, variables }))
    await dispatch(receiveBaseGtfs({...data, partial: true}))
    if (lockCreated) {
      await dispatch(removeEditorLock(feedSource.id, false, true))
    }
  }
}
...
export function saveEntity (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (data.location_group_id && data.location_id) {
      data.location_id = data.location_id.join(',')
      if (data.is_creating) {
        delete data.is_creating
        delete data.id
      }
    }
