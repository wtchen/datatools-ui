...
const settingActiveGtfsEntity = createAction(
...
function getActiveSubEntity (activeEntity: ?Entity, subEntityId: ?number): any {
  return activeEntity && activeEntity.tripPatterns
...
function updateUrl (
  ...
  if (!feedSourceId) {
    ...
    console.warn('entityId not defined, could not calculate url')
    return
  } else if (subComponentList.indexOf(subComponent) === -1) {
    ...
    console.warn('subEntityId not defined, could not calculate url')
    return
  } else if (subSubComponentList.indexOf(subSubComponent) === -1) {
