...
import {updateActiveGtfsEntity} from '../active'
import {setActivePatternSegment} from '../tripPattern'
import {setErrorMessage} from '../../../manager/actions/status'
import {getLineSlices, recalculateSegment, newControlPoint, isSegmentActive} from '../../util/map'
...
export function constructControlPoint ({pattern, latlng, controlPoints, segmentIndex, segmentCoordinates, patternCoordinates}) {
  return function (dispatch, getState) {
    ...
    const controlPoint = newControlPoint(distTraveled, pointOnLine, false, {
      ...
      pointType: 1 // FIXME replace with constant
    })
...
export function handleControlPointDrag (
  ...
  return function (dispatch, getState) {
    ...
    recalculateSegment({
      ...
      const stillDragging = result.dragId === currentDragId
      if (
        stillDragging &&
        result.coordinates
      ) {
