...
const addRangeToArray = (array: Array<number>, startIndex: number, lengthToAdd: number): Array<number> => {
  const range = []
  for (let i = 0; i < lengthToAdd; i++) { array.push(startIndex + 1) }
  return range
}
...
const generateSegmentAndInsert = async (segments: Array<Coordinates>, fromPoint: ?ControlPoint, toPoint: ?ControlPoint, insertionPoint: number) => {
  ...
  if (segment && segment.coordinates) segments[insertionPoint] = segment.coordinates
}
...
const updateShapeDistTraveled = (controlPoints: Array<ControlPoint>, segments: Array<Array<number>>): void => {
  ...
  stopControlPoints.forEach((cp, index) => {
    if (index === 0) controlPoints[cp.cpIndex].shapeDistTraveled = 0.0
      ...
      let newShapeDistTraveled = controlPoints[previousStopCP.cpIndex].shapeDistTraveled
...
export function updateShapesAfterPatternReorder (oldPattern: Pattern, newPatternStops: Array<PatternStop>, oldPatternStopSequence: number) {
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    let previousFromStopControlPoint
    let previousToStopControlPoint
