...
import type {ControlPoint, GtfsStop, LatLng, Pattern, PatternStop} from '../../../types'
 ...
 */
const addRangeToArray = (array, startIndex, numberToAdd): Array<Number> => {
 ...
 * for later deletion.
 */
const updateControlPointsAndSegments = (deletedControlPoints, deletedSegments, deletionStartIndex, numberSegments) => {
 ...
 */
const generateSegmentAndInsert = async (clonedPatternSegments, fromPoint, toPoint, insertionPoint) => {
  ...
  clonedPatternSegments[insertionPoint] = segment.coordinates
}
 ...
 */
const updateShapeDistTraveled = (controlPoints, segments) => {
...
export function updateShapesAfterPatternReorder (oldPattern: Pattern, newPatternStops: Array<PatternStop>, oldPatternStopSequence: number) {
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    let deletedControlPoints = []; let deletedSegments = []
    ...
    if (!movedFromEnd) { // Delete old "to" segment control points and segments, no "to" segment if we're moving from the end
      previousToStopControlPoint = !movedFromEnd && stopControlPoints[oldPatternStopSequence + 1]
    ...
    if (!movedFromStart) { // Delete old "from" segment control points and segments, no "from" segment if we're moving from the start
      previousFromStopControlPoint = !movedFromStart && stopControlPoints[oldPatternStopSequence - 1] // Null if moved from beginning
    ...
    try {
      ...
      if (!movedFromStart && !movedFromEnd) {
    ...
    // TODO: there seems to be something fishy going on with the actual saving of these values --> investigate this 🔎
