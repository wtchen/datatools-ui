  ...
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    const newStop = stopToPatternStop(
      ...
      (typeof index === 'undefined' || index === null)
        ? patternStops.length + patternLocations.length
        : index
    )

    let patternHalts = mergePatternHalts(patternStops, patternLocations)
    ...
    if (typeof index === 'undefined' || index === null || index === patternHalts.length) {
      ...
      if (hasShapePoints && !!stop.stop_lat && !!stop.stop_lon) {
        ...
        if (newStop.hasOwnProperty('stopId')) patternStops.push(newStop)
        patternHalts = mergePatternHalts(patternStops, patternLocations)
        ...
        // $FlowFixMe Flow doesn't understand type check
        if (newStop.hasOwnProperty('locationId')) patternLocations.push(newStop)
        patternHalts = mergePatternHalts(patternStops, patternLocations)
...
export function removeStopFromPattern (pattern: Pattern, stop: GtfsLocation | GtfsStop, index: number) {
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    const patternLocations = [...pattern.patternLocations]
    const patternHalts = mergePatternHalts(patternStops, patternLocations)
    ...
    if (stopIsStop && (!shapePoints || shapePoints.length === 0)) {
      ...
      // Update pattern geometry
      stopIsStop && dispatch(updatePatternGeometry({
        controlPoints: result.updatedControlPoints,
        patternSegments: result.coordinates
      }))
    }
