    ...
    if (typeof index === 'undefined' || index === null) {
      ...
      if (coordinates) {
        ...
        if (patternStops.length > 1) {
          ...
          const previousStop = stops.find(s => s.id === patternStops[patternStops.length - 2].stopId)
          ...
          .then(geojson => {
            dispatch(updateActiveGtfsEntity(pattern, 'trippattern', {
              patternStops,
              shape: {type: 'LineString', coordinates: geojson.coordinates}
            }))
            return dispatch(saveActiveGtfsEntity('trippattern'))
          })
        } else {
  ...
  return async function (dispatch, getState) {
    ...
    if (!patternCoordinates || patternCoordinates.length === 0) {
      ...
      const cpIndex = controlPoints.findIndex(cp => {
        // Increment control point stop index if cp is references a stop
