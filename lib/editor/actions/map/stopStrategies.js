 ...
 */
const generateSegmentAndInsert = async (segments: Array<Coordinates>, fromPoint: ?ControlPoint, toPoint: ?ControlPoint, insertionPoint: number) => {
  ...
  if (segment && segment.coordinates) segments.splice(insertionPoint, 0, segment.coordinates)
}

/**
 * Updates the shape_dist_traveled values for each control point after a change in the pattern has occurred. These values must also be updated in the
 * patternStops which will be used to create the shapeDistTraveled values in stop_times.txt
 */
const updateShapeDistTraveled = (controlPoints: Array<ControlPoint | null>, segments: Array<Coordinate>, patternStops: Array<PatternStop>): void => {
  ...
  stopControlPoints.forEach((cp, index) => {
    let newShapeDistTraveled
    const patternStopIdx = patternStops.findIndex(el => el.stopId === cp.stopId)
    if (index === 0) newShapeDistTraveled = 0.0
    else {
    ...
    // $FlowFixMe
    controlPoints[cp.cpIndex].distance = newShapeDistTraveled // FixMe? distance should be equal to the shapeDistTraveled for a stop... it was slightly different before.
