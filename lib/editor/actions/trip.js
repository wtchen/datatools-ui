...
export function saveTripsForCalendar (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    // --- Perform a test call to the new endpoint --- //
    const testUrl = `/api/editor/secure/trip/updatetrips?feedId=${feedId}&sessionId=${sessionId}`
    dispatch(secureFetch(testUrl, 'POST', {trips, name: 'test request'}))
      .then(res => res.json())
      .catch(err => {
        console.warn(err)
