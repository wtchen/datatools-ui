...
class EntityDetailsHeader extends Component<Props> {
  ...
  render () {
    ...
    const emptyTripPattern = activeEntity.tripPatterns && activeEntity.tripPatterns.some(pattern => pattern.patternLocations.length === 0 && pattern.patternStops.length === 0)
    // flex zoom available only when a trip pattern is actually rendered on the map
    const tripPatternRendered = activeEntity.tripPatterns && !(this.props.params ? this.props.params.subEntityId : false)
...
const ActiveEntityDetailsHeader = connect(mapStateToProps, mapDispatchToProps)(EntityDetailsHeader)
