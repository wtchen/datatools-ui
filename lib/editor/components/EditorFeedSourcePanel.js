...
import type {ManagerUserState, LockState} from '../../types/reducers'
...
export default class EditorFeedSourcePanel extends Component<Props> {
  ...
  _openModal = () => {
    const {lock, feedSource, lockEditorFeedSource, removeEditorLock} = this.props
    // Get lock if we don't have one.
    // Snapshot Modal requires lock to fetch unapproved routes.
    if (!lock.sessionId) {
      lockEditorFeedSource(feedSource.id).then((data) => {
        // Clean up lock on close.
        const onClose = () => removeEditorLock(feedSource.id)
        this.refs.snapshotModal.getWrappedInstance().open(onClose)
      })
    } else {
