...
export default class ExceptionDateRange extends Component<Props, State> {
  ...
  _onRangeUpdate = (millis: number | string, isStartDate: boolean) => {
    ...
    if (
      ...
      this.setState({validationStateOverride: true})
      return
    }
    ...
    this.setState({validationStateOverride: false}, () => {
  ...
  render () {
    ...
    return (
      ...
      <FormGroup validationState={validationState === 'error' || this.state.validationStateOverride ? 'error' : 'success'}>
        <div
          key={index}
          style={{
            display: 'grid',
