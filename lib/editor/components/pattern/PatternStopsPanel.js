...
import type {Pattern, GtfsLocation, GtfsStop, GtfsStopArea, Feed, ControlPoint, Coordinates} from '../../../types'
...
type State = {
  patternStopCandidate: ?GtfsStop | ?GtfsLocation,
...
export default class PatternStopsPanel extends Component<Props, State> {
  ...
  _selectStop = (input: any) => {
    ...
    const stop: GtfsStop | GtfsLocation = input.entity
  ...
  render () {
    ...
    const patternHasStops = (activePattern.patternStops &&
      activePattern.patternStops.length > 0) || (activePattern.patternLocations &&
        activePattern.patternLocations.length > 0) || (activePattern.patternStopAreas &&
          activePattern.patternStopAreas.length > 0)

    const patternHaltCount = (activePattern.patternStops.length || 0) + (activePattern.patternLocations.length || 0) + (activePattern.patternStopAreas.length || 0)
    return (
      <div>
          ...
          {addStops
            ? <div className='pattern-stop-card'>
              <VirtualizedEntitySelect
                ...
                // Override the id of locations and location groups since their ids may clash with stops
                entities={[...stops, ...locations.map(l => { return {...l, id: uuidv4()} }), ...stopAreas.map(sa => { return {...sa, id: uuidv4()} })]}
                // TODO make it understand non-stops
