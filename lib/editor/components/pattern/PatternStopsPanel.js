  ...
  render () {
    ...
    const patternHasStops = (activePattern.patternStops &&
      activePattern.patternStops.length > 0) || (activePattern.patternLocations &&
        activePattern.patternLocations.length > 0)

    const patternHaltCount = (activePattern.patternStops.length || 0) + (activePattern.patternLocations.length || 0)
          ...
          {addStops
            ? <div className='pattern-stop-card'>
              <VirtualizedEntitySelect
                ...
                entities={[...stops, ...locations.map(l => { return {...l, id: uuidv4()} }), ...locationGroups.map(lg => { return {...lg, id: uuidv4()} })]}
