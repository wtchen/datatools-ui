...
// FLEX TODO: is this too much of a hack?
// Method to avoid ID overlap -- adds a prefix to each ID.
const unifyLocationAndStopIds = (prefix: number) => {
  return function (element: GtfsStop | GtfsLocation) {
    element.id = prefix + element.id
    return { id_prefix: prefix, ...element }
  }
}
...
export default class PatternStopsPanel extends Component<Props, State> {
  ...
  _selectStop = (input: any) => {
    ...
    // Account for the ID prefix we added earlier
    if (input.id_prefix) input.id = input.id - input.id_prefix
  ...
  render () {
    ...
    const unifiedStopsAndLocations = [
      ...stops.map(unifyLocationAndStopIds(100)),
      ...locations.map(unifyLocationAndStopIds(200))
    ]
    return (
      <div>
          ...
          {addStops
            ? <div className='pattern-stop-card'>
              <VirtualizedEntitySelect
                ...
                // TODO add locations
                entities={unifiedStopsAndLocations}
