...
class PatternRow extends Component<RowProps> {
  ...
  render () {
    ...
    const {name, patternLocations, patternStopAreas, patternStops} = pattern
    const rowStyle = {
      paddingTop: 5,
      paddingBottom: 5
    }
    ...
    if (name) {
      const showStopCount = patternStops || patternLocations || patternStopAreas
      const stopCount = showStopCount ? (
        (patternStops ? patternStops.length : 0) +
        (patternLocations ? patternLocations.length : 0) +
        (patternStopAreas ? patternStopAreas.length : 0)
      ) : 0
