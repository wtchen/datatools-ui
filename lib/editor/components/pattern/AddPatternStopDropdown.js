...
import type {GtfsLocation, GtfsStop, Pattern, PatternStop, Style} from '../../../types'
...
export default class AddPatternStopDropdown extends Component<Props> {
  ...
  _matchesStopAtIndex = (index: number) => {
    ...
    // Support both stop and location IDs
    let unifiedId
    if (stop.stop_id) unifiedId = stop.stop_id
    if (stop.location_id) unifiedId = stop.location_id

    return patternStopAtIndex && patternStopAtIndex.stopId === unifiedId
  }
