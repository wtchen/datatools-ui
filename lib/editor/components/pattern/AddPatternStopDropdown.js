...
import type {GtfsLocation, GtfsStop, Pattern, PatternLocation, PatternStop, Style} from '../../../types'
...
type Props = {
  ...
  label?: string,
  patternStop?: PatternStop | PatternLocation, // optional current pattern stop
  size?: string,
...
export default class AddPatternStopDropdown extends Component<Props> {
  ...
  _matchesStopAtIndex = (index: number): boolean => {
      ...
      >
        ...
        <Dropdown.Menu style={{maxHeight: '200px', overflowY: 'scroll'}}>
          <MenuItem
            disabled={addToEndDisabled}
            title={addToEndDisabled ? DISABLED_MESSAGE : ''}
            value={patternHalts.length}
            eventKey={patternHalts.length}>
            Add to end (default)
          </MenuItem>
          {patternHalts && patternHalts.map((s, i) => {
            ...
            // - nextIndex = 4
            // - previousIndex = 2
            const addIndex = patternHalts.length - i - 1
