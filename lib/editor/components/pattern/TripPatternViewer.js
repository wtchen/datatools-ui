...
export default class TripPatternViewer extends Component<Props> {
  ...
  render () {
    ...
    // FLEX TODO: Is this the best place to put it?
    activePattern.patternLocations = activePattern.patternLocations.map(pl => { return {...pl, id: uuidv4()} })
    activePattern.patternStops = activePattern.patternStops.map(ps => { return {...ps, id: uuidv4()} })
    return (
      <div>
        <EditableTextField
