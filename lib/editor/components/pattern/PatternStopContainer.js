...
import type {Feed, GtfsLocation, GtfsStop, GtfsStopArea, Pattern, PatternHalt} from '../../../types'
import { mergePatternHaltsOfPattern } from '../../../gtfs/util'
import {
  patternHaltIsLocation,
  patternHaltIsStopArea
} from '../../util/location'
    ...
    return connectDropTarget(
      <div>
        {cards.map((card, i) => {
          ...
          if (patternHaltIsStopArea(card)) {
            // $FlowFixMe flow doesn't appreciate our type check
            stop = stopAreas.find(area => area.area_id === card.areaId)
            // $FlowFixMe flow doesn't appreciate our type check
            cumulativeTravelTime += card.flexDefaultZoneTime + card.flexDefaultTravelTime
          }
