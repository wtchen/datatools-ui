...
import type {GtfsLocation, GtfsStop, Feed, Pattern, PatternLocation, PatternStop} from '../../../types'
...
type State = {
  cards: Array<PatternStop | PatternLocation>
}
...
class PatternStopContainer extends Component<Props, State> {
  componentWillMount () {
    ...
    this.setState({
      cards: mergePatternHalts(patternStops, patternLocations)
    })
  }

  componentWillReceiveProps (nextProps) {
    // Updates cards when pattern stops order changes (does not account for changes to default travel/dwell times)
    const {patternLocations, patternStops} = nextProps.activePattern
    if (
      (this.props.activePattern.patternStops &&
        !shallowEqual(this.props.activePattern.patternStops, patternStops)
      ) ||
      (this.props.activePattern.patternLocations &&
        !shallowEqual(this.props.activePattern.patternLocations, patternLocations)) ||
        this.props.activePattern.patternLocations.length !== patternLocations.length
    ) {
      this.setState({
        cards: mergePatternHalts(patternStops, patternLocations)
      })
  ...
  render () {
    ...
    const stopNotFound = mergePatternHalts(activePattern.patternStops, activePattern.patternLocations)
