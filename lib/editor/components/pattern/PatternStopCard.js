...
class PatternStopCard extends Component<Props> {
  _formatTravelTime (cumulativeTravelTime, patternStop) {
    ...
    // $FlowFixMe Flow does not recognize the "type check" above
  ...
  render () {
    ...
    const stopType = (stop.stop_id && 'stop') || (stop.meanDurationFactor && 'location')
...
class PatternStopContents extends Component<Props, State> {
  componentWillMount () {
    ...
    if (patternStop.hasOwnProperty('defaultDwellTime')) {
      this.setState({
    ...
    } else if (patternStop.hasOwnProperty('flexWindowOffsetStart')) {
      this.setState({
        // $FlowFixMe flow doesn't like our "type check"
