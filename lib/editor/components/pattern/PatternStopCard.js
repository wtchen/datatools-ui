...
class PatternStopContents extends Component<Props, State> {
  componentWillMount () {
    this.setState({
      ...
      flexWindowOffsetStart: 888, // TODO: Pull correct value
      flexWindowOffsetEnd: 999, // TODO: Pull correct value
      meanDurationFactor: 999, // TODO: Pull correct value
      meanDurationOffset: 999, // TODO: Pull correct value
      safeDurationFactor: 999, // TODO: Pull correct value
      safeDurationOffset: 999, // TODO: Pull correct value
      update: false
    })
  }
  ...
  _onTextFieldChange = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    ...
    const {activePattern, index, updatePatternStops} = this.props
    // FLEX TODO: use patternLocations if set
    const patternStops = [...activePattern.patternStops]
    // FLEX TODO: Either here or inside updatePatternStops all offsets needs to be converted
    // to an actual value
    patternStops[index][evt.target.id] = evt.target.value
    // FLEX TODO: can this explicit update be removed once the server knows what's going on?
    this.setState({[evt.target.id]: evt.target.value, update: true})
    updatePatternStops(activePattern, patternStops)
  }
  ...
  _onBookingRuleChange = (change: {field: string, label: string, value: string}) => {
    const selectedRuleId: string = change.value
    const {activePattern, index, updatePatternStops} = this.props
    const patternStops = [...activePattern.patternStops]

    const newPatternStop = clone(patternStops[index])
    newPatternStop[change.field] = selectedRuleId
    patternStops[index] = newPatternStop
    this.setState({update: true})
    updatePatternStops(activePattern, patternStops)
  }
  ...
  render () {
      ...
      <div>
        ...
        <Row>
          ...
          <Col xs={6}>
            ...
            <FormControl id={'meanDurationOffset'}
    ...
    let innerDiv
    if (active) {
      // TODO: use locationRows instead of stopRows if applicable
      innerDiv = <div>
        <Row>
          <Col xs={4}>
            <Checkbox
        ...
        {stopRows}
        {/* FLEX TODO: add locationRow if applicable */}
