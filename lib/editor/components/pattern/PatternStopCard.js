...
import type { Feed, Pattern, PatternLocation, PatternLocationGroup, PatternStop } from '../../../types'
...
type Props = {
  ...
  feedSource: Feed,
  findCard: string => { card: PatternStop | PatternLocation | PatternLocationGroup, index: number },
  id: string,
  index: number,
  isDragging: boolean,
...
class PatternStopContents extends Component<Props, State> {
  refreshFields (nextProps) {
    ...
    } else if (patternHaltIsLocation(patternStop) || patternHaltIsLocationGroup(patternStop)) {
      this.setState({
        // $FlowFixMe flow doesn't like our "type check"
  ...
  render () {
      ...
      <Row>
        <Col xs={6}>
          <PickupDropoffSelect
            ...
            value={patternStop.pickupType || 1}
          />
        </Col>
        <Col xs={6}>
          <PickupDropoffSelect
            ...
            value={patternStop.dropOffType || 1}
          />
        </Col>
      </Row>
    )
    ...
    const stopRows = (
      <div>
        ...
        {feedSource.flexUIFeaturesEnabled && pickupDropOffRow}
        {feedSource.flexUIFeaturesEnabled && (
          <Row>
            <Col xs={6}>
              <PickupDropoffSelect
                activePattern={activePattern}
    ...
    if (active) {
      innerDiv = (
        <div>
          ...
          {(patternHaltIsLocation(patternStop) || patternHaltIsLocationGroup(patternStop)) && locationRows}
