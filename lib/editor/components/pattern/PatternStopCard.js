...
const cardSource = {
  ...
  canDrag (props, monitor) {
    if (props.status.savePending) {
      ...
      console.warn(`Cannot drag card (id=${id}). Save is in progress.`)
      return false
    }
    return true
  }
...
class PatternStopCard extends Component {
  ...
  render () {
    ...
    const titleStopName = stop ? `${index + 1}. ${stopName}` : `${index + 1}. [unknown stop]`
    const fullStopName = stop ? `${index + 1}. ${abbreviatedStopName}` : `${index + 1}. [unknown stop]`
