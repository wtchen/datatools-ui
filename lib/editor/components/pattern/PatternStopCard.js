...
class PatternStopContents extends Component<Props, State> {
  ...
  _onPickupOrDropOffChange = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    const selectOptionType: string = evt.target.id
    const selectedOptionValue: number = parseInt(evt.target.value)

    const {activePattern, index, updatePatternStops} = this.props
    const patternStops = [...activePattern.patternStops]

    switch (selectOptionType) {
      case 'continuous_pick-up':
        patternStops[index].continuousPickup = selectedOptionValue
        break
      case 'continuous_drop_off':
        patternStops[index].continuousDropOff = selectedOptionValue
        break
      case 'pickup_type':
        patternStops[index].pickupType = selectedOptionValue
        break
      case 'drop_off_type':
        patternStops[index].dropOffType = selectedOptionValue
        break
      default:
        return null
    }
