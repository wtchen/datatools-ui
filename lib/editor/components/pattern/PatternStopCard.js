...
class PatternStopCard extends Component<Props> {
  ...
  render () {
    ...
    // FLEX TODO: a better type for this? an enum maybe?
    const stopType = (stop.stop_id && 'stop') || (stop.location_id && 'location')
    return connectDragSource(connectDropTarget(
      <div
        ...
        <PatternStopContents {...this.props} stopType={stopType} />
      </div>
    ))
  }
}
...
class PatternStopContents extends Component<Props, State> {
  ...
  render () {
    const {active, activePattern, feedSource, patternEdited, patternStop, tables, stopType} = this.props
    ...
    if (active) {
      ...
      innerDiv = <div>
        ...
        {stopType === 'stop' && stopRows}
        {stopType === 'location' && locationRows}
