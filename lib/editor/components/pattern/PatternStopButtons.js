...
import type {Feed, GtfsLocation, GtfsStop, Pattern, PatternHalt, Style} from '../../../types'
...
export default class PatternStopButtons extends Component<Props> {
  ...
  _onClickEdit = () => {
    ...
    } else if (stop.hasOwnProperty('area_id')) {
      setActiveEntity(feedSource.id, 'stoparea', stop)
  ...
  render () {
    ...
    const haltType = stop.hasOwnProperty('area_id') ? 'stop area' : stop.hasOwnProperty('location_id') ? 'location' : 'stop'
    return (
      <ButtonGroup
