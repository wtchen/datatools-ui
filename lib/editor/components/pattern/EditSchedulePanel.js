...
export default class EditSchedulePanel extends Component {
  ...
  _onChangeDirection = (evt) => {
    const patternDirection = typeof evt.target.value === 'undefined' ? +!this.props.activePattern.patternDirection : +evt.target.value
    if (this.props.activePattern.patternDirection !== patternDirection && DIRECTIONS.indexOf(patternDirection) !== -1) {
          ...
          <ButtonGroup className='pull-right'>
            {DIRECTIONS.map(dir => (
              <Button
                ...
                >
                <Icon
                  value={dir} // needs value prop in case onClick triggered by icon component
                  type={dir === 0 ? 'sign-out' : 'sign-in'} // 0 === outbound
                />
              </Button>
            ))}
          </ButtonGroup>
