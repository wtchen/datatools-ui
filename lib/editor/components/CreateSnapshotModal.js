...
class CreateSnapshotModal extends Component<Props, State> {
  ...
  render () {
    ...
    const unapprovedRoutes = routeStatusDict ? routes.filter(route => route.hasOwnProperty('status') && route.status !== ROUTE_STATUS_CODES.APPROVED) : []
              ...
              <ul>
                {
                  unapprovedRoutes.map(route => (
                    <li key={route.route_id}>{getEntityName(route)}: {routeStatusDict[parseInt(route.status)].text}</li>
                  ))
                }
              </ul>
              <FormGroup>
