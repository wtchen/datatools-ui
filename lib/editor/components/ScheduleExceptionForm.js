...
import {createDateRangesFromDates, filterOutNewDates} from '../../common/util/exceptions'
import {EXCEPTION_EXEMPLARS} from '../util'
import {getTableById} from '../util/gtfs'
import type {ParsedRangesAndDates, ServiceCalendar, ScheduleException} from '../../types'
...
export default class ScheduleExceptionForm extends Component<Props> {
  ...
  // $FlowFixMe: fix return type
  _parseDatesForRanges = (dates: Array<string | 0>): ParsedRangesAndDates => {
    if (dates.length === 1) return {ranges: [], parsedDates: [{date: dates[0], index: 0}]}

    // $FlowFixMe Flow does not properly type a reducer
    const newDates = dates.reduce((acc, cur, curIndex) => {
      if (typeof cur === 'number') acc.push({date: 0, index: curIndex})
      return acc
    }, [])

    dates = filterOutNewDates(dates)
    dates.sort((a, b) => moment(a).diff(moment(b)))

    // $FlowFixMe Flow does not understand proper typing for array#reduce
    const {ranges, dates: parsedDates} = dates.reduce(createDateRangesFromDates, {ranges: [], dates: []})

    return {
      ranges,
      parsedDates: [...parsedDates, ...newDates]
    }
  }
  ...
  _checkRangeValidation = (startIndex: number, endIndex: number) => {
    // Rather than loop through indices, create an array of indices to consolide checks
    const rangeIndices = Array.from({length: endIndex - startIndex + 1}, (el, index) => startIndex + index)
    return rangeIndices.some(index => Boolean(this._checkValidation(`dates-${index}`)))
  }
  ...
  render () {
    ...
    const {ranges, parsedDates} = this._parseDatesForRanges(dates)
          ...
          >
            ...
            {ranges && ranges.length !== 0 && ranges.map((range, index) => (
              <ExceptionDateRange
                ...
                key={index}
                range={range}
                isNotValid={this._checkRangeValidation(range.startIndex, range.endIndex)}
                {...this.props}
              />
            ))}
            {parsedDates && parsedDates.length !== 0 && parsedDates.map((date, index) => (
              <ExceptionDate
                index={date.index !== undefined ? date.index : index}
                key={index}
                date={date.date}
                isNotValid={Boolean(this._checkValidation(`dates-${date.index}`))}
                {...this.props}
              />
            ))}
