          ...
          >
            ...
            {ranges && ranges.map((range, index) => (
              <ExceptionDateRange
                ...
                key={index}
                range={range}
                isInvalid={this._checkRangeValidation(dates.findIndex(date => date === range.startDate), dates.findIndex(date => date === range.endDate))}
                {...this.props}
              />
            ))}
            {parsedDates && parsedDates.map((date, index) => (
              <ExceptionDate
                ...
                key={index}
                date={date}
                isInvalid={Boolean(this._checkValidation(`dates-${dates.findIndex(d => d === date)}`))}
                {...this.props}
              />
            ))}
