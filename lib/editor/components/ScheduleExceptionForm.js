...
export default class ScheduleExceptionForm extends Component<Props> {
  ...
  render () {
    ...
    // getRangesForDates requires a sorted array.
    const {ranges, parsedDates} = getRangesForDates([...dates]).reduce((acc, cur) => {
      if (cur.startDate === cur.endDate) acc.parsedDates.push(cur.startDate)
      else acc.ranges.push(cur)
      return acc
    }, {ranges: [], parsedDates: []})

    // Sort the zeroes to the end of the array for display purposes.
    parsedDates.sort((a: any, b: any) => {
      if (a === 0) return 1
