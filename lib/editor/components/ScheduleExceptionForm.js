...
export default class ScheduleExceptionForm extends Component {
  ...
  _onAddedServiceChange = input => {
    const {activeComponent, activeEntity, updateActiveEntity} = this.props
    updateActiveEntity(activeEntity, activeComponent, {added_service: this._mapInput(input)})
  }
  ...
  _onCustomScheduleChange = input => {
    const {activeComponent, activeEntity, updateActiveEntity} = this.props
    updateActiveEntity(activeEntity, activeComponent, {custom_schedule: this._mapInput(input)})
  }
  ...
  _onRemovedServiceChange = input => {
    const {activeComponent, activeEntity, updateActiveEntity} = this.props
    updateActiveEntity(activeEntity, activeComponent, {removed_service: this._mapInput(input)})
  }
      ...
      <div>
        <Form>
          ...
          activeEntity.exemplar === EXCEPTION_EXEMPLARS.SWAP
            ? (
              <FormGroup controlId={`swap`} className={`col-xs-12`}>
                <ControlLabel>
