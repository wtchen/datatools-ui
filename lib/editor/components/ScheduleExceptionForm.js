...
export default class ScheduleExceptionForm extends Component<Props> {
  ...
  render () {
    ...
    let addDateDisabled = false // Disable adding a new date if we come across any current validation issues.
    const {ranges, parsedDates} = getRangesForDates([...dates].sort((a: any, b: any) => {
      ...
      let validationState
      const dateIndex = dates.findIndex(d => d === cur.startDate)
      if (cur.startDate === cur.endDate) {
        validationState = this._checkValidation(`dates-${dateIndex}`)
        acc.parsedDates.push({
          date: cur.startDate,
          validationState
        })
      } else {
        const endIndex = dates.findIndex(d => d === cur.endDate)
        validationState = this._checkRangeValidation(dateIndex, endIndex)
        acc.ranges.push({
          ...cur,
          validationState
        })
      ...
      if (validationState === 'error') addDateDisabled = true
      return acc
    }, {ranges: [], parsedDates: []})
            ...
            {ranges && ranges.map((range, index) => {
              return (
                <ExceptionDateRange
                  ...
                  validationState={range.validationState}
                  {...this.props}
                />)
            })}
            {parsedDates && parsedDates.map((date, index) => {
              return (
                <ExceptionDate
                  ...
                  index={index}
                  key={index}
                  validationState={date.validationState}
                  {...this.props}
                />)
            })}
