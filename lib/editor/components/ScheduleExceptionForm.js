...
import type {EditorTableData, GtfsCalendar, ScheduleException} from '../../types'
import type {EditorValidationIssue} from '../util/validation'

type Props = {
  activeEntity: ScheduleException,
  updateActiveEntity: (any, string, any) => void,
...
export default class ScheduleExceptionForm extends Component<Props> {
  ...
  _filterByList = (calendars: Array<GtfsCalendar>, list: ?Array<string>) => calendars
    .filter(cal => !list || list.indexOf(cal.service_id) === -1)

  _mapInput = (input: ?Array<SelectOption>): ?Array<GtfsCalendar> =>
  ...
  render () {
    ...
    const calendars = (getTableById(tableData, 'calendar'): Array<GtfsCalendar>)
    const {
      exemplar,
      name,
