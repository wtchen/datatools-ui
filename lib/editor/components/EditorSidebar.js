...
export default class EditorSidebar extends Component<Props> {
  isActive (item: GtfsIcon, component: string) {
    return (component === item.id) || (component === 'scheduleexception' && item.id === 'calendar') || (component === 'location' && item.id === 'stop')
  }
    ...
    return (
      <ActiveSidebar>
        <ActiveSidebarNavItem
          ...
          ref='backNav'
        />
        {GTFS_ICONS.filter(item => !item.flexOnly || (item.flexOnly && feedSource && feedSource.flexUIFeaturesEnabled)).map(item => {
          return item.hideSidebar
            ...
            : <ActiveSidebarNavItem
              ...
              label={item.id === 'stop' && feedSource && feedSource.flexUIFeaturesEnabled ? 'Stops & Locations' : item.label}
              link={!feedSource
                ? '/home'
