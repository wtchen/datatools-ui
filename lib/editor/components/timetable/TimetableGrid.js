...
type CellRenderProps = { columnIndex: number, key: any, rowIndex: number, style: Object }
type Props = { /* ... */ }
...
export default class TimetableGrid extends Component {
  ...
  _renderLeftHeaderCell = ({ columnIndex, key, rowIndex, style }: CellRenderProps) => (
  ...
  _renderLeftColumnCell = ({ columnIndex, key, rowIndex, style }: CellRenderProps) => (
  ...
  _renderHeaderCell = ({ columnIndex, key, rowIndex, style }: CellRenderProps) => {
  ...
  handlePastedRows = (pastedRows: Array<Array<string>>, rowIndex: number, colIndex: number) => {
  ...
  _cellRenderer = ({ columnIndex, key, rowIndex, style }: CellRenderProps) => {
    ...
    if (col.key === 'gtfsTripId' && val === null) {
      val = objectPath.get(data[rowIndex], 'id') !== 'new' ? objectPath.get(data[rowIndex], 'id') : -1
    }
    const previousValue = previousCol && row && objectPath.get(row, previousCol.key)
    const isInvalid = isTimeFormat(col.type) && val >= 0 && val < previousValue && val !== -1
    return (
      <EditableCell
        key={key}
  ...
  _onCellChange = (value: string | number, rowIndex: number, col: Object, colIndex: number) => {
  ...
  handleEndEditing = () => {
    ...
    const el = ReactDOM.findDOMNode(this.grid)
  ...
  render () {
    ...
    const {style, data, columns, hideDepartureTimes} = this.props
