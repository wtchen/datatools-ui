...
type Props = {
  ...
  showHelpModal: () => void,
  showTimetableContextMenu: typeof tripActions.showTimetableContextMenu,
  style: Style,
...
export default class TimetableGrid extends Component<Props> {
  ...
  _renderLeftHeaderCell = (cellProps: CellProps) => {
    const {showTimetableContextMenu, toggleAllRows} = this.props
    return (
      <HeaderCell // This is the select all checkbox
        key={cellProps.key}
        onChange={toggleAllRows}
        showTimetableContextMenu={showTimetableContextMenu}
  ...
  _renderLeftColumnCell = (cellProps: CellProps & {selected: Array<number>}) => {
    const {selected, showTimetableContextMenu, toggleRowSelection} = this.props
    return (
      <HeaderCell // Select row checkbox
        active={selected.indexOf(cellProps.rowIndex) !== -1}
        key={cellProps.key}
  ...
  _renderHeaderCell = (cellProps: CellProps) => {
    const {columns, data, selected, showTimetableContextMenu} = this.props
    const col = getHeaderColumns(columns)[cellProps.columnIndex]
    return (
      <HeaderCell
        key={cellProps.key}
        // TODO: Apply column index here.
        // render column headers as active if all rows selected
        active={selected.length > 0 && selected.length === data.length}
