...
export default class TimetableGrid extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    // TODO: Change focus to grid if a trip is added.
    // if (nextProps.data.length > this.props.data.length) {
    //   console.log(`trip was added`, nextProps)
    //   // Focus on grid if a trip is added.
    //   this._focusOnGrid()
    // }
  }
  ...
  _handleKeyPress = (evt) => {
    ...
    // // input was 0-9
    // if (!activeCell && evt.keyCode >= 48 && evt.keyCode <= 57 && !ctrlPressed) {
    //   setActiveCell(`${scrollToRow}-${scrollToColumn}`)
    // }
    // // input was a-z
    // if (!activeCell && evt.keyCode >= 65 && evt.keyCode <= 90 && !ctrlPressed) {
    //   setActiveCell(`${scrollToRow}-${scrollToColumn}`)
    // }
  }
  ...
  _renderLeftColumnCell = ({ columnIndex, key, rowIndex, style, selected }) => (
   ...
   */
  _handleCellChange = (value, rowIndex, col, colIndex) => {
    const {
      activePattern,
      columns,
      data,
