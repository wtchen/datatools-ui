...
import type {TimetableState} from '../../reducers/timetable'
...
type Props = {
  ...
  updateCellValue: ({value: any, rowIndex: number, key: string}) => void,
...
export default class TimetableGrid extends Component<Props> {
    ...
    switch (evt.keyCode) {
      ...
      case 13: // Enter
        if (!activeCell) {
          ...
          // If cell is active, set to inactive and move up or down (with shift)
          // a row.
          setActiveCell(null)
  ...
  _renderLeftColumnCell = ({ columnIndex, key, rowIndex, style, selected }: { columnIndex: number, key: string, rowIndex: number, style: Style, selected: Array<number>}) => (
  ...
  _renderHeaderCell = ({ columnIndex, key, rowIndex, style }: { columnIndex: number, key: string, rowIndex: number, style: Style}) => {
  ...
  _cellRenderer = ({ columnIndex, key, rowIndex, style }: { columnIndex: number, key: string, rowIndex: number, style: Style}) => {
    const {
      activeCell,
      columns,
      data,
    ...
    const isInvalid = isTimeFormat(col.type) && typeof val === 'number' && val >= 0 && val < previousValue && val !== null
    return (
      <EditableCell
  ...
  render () {
    ...
    const scrollSize: number = scrollbarSize()
              ...
              <div>
                ...
                <div
                  ...
                  <Grid // Top Header Row
                    ...
                    width={width - scrollSize - LEFT_COLUMN_WIDTH} />
                </div>
                <div
