...
export default class HeaderCell extends Component<Props, State> {
  ...
  _handleClick = (e: SyntheticMouseEvent<HTMLInputElement>) => {
    this._selectCell()
  }
  ...
  _handleRightClick = (e: SyntheticMouseEvent<HTMLInputElement>) => {
    if (!this.state.active) this._selectCell()
  ...
  _selectCell () {
    const {index, onChange, selectable} = this.props
    if (selectable) {
