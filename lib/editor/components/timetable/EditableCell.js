...
const normalizeOver24HourTime = (hours) => [String(hours % 24).padStart(2, '0'), Math.floor(hours / 24)]
...
export default class EditableCell extends Component<Props, State> {
  ...
  save () {
    ...
    if (column.type === 'TEXT') {
      ...
      // for time rendering
      // FIXME replace with secondsAfterMidnightToHHMM?
      if (typeof data !== 'string') return this.cancel()
      let time = data
      let hours; let days = 0
      let parts = []
      if (time.includes(':')) {
        // is a string with colons
        parts = time.split(':');
        [hours, days] = normalizeOver24HourTime(+parts[0])
        parts[0] = hours
      } else {
        // is a string without colons
        if (time.length === 3) {
          // format of `hmm`
          parts = [time[0], time.substr(1, 3)]
          hours = +time[0]
        } else if (time.length === 4) {
          // format of `HHmm`
          [hours, days] = normalizeOver24HourTime(+time.substr(0, 2))
          parts = [hours, time.substr(2, 4)]
        }
      }
      time = parts.join(':')

      const date = moment().startOf('day').format('YYYY-MM-DD')
      const momentTime = moment(date + 'T' + time, TIMETABLE_FORMATS, true)
      let value = momentTime.isValid() ? momentTime.diff(date, 'seconds') : null

      if (days > 0 && momentTime.isValid()) {
        value += 86400 * days
      }
      // Check for valid time and new value
      const timeIsValid = (time === '' || momentTime.isValid()) && value !== time
      if (timeIsValid) this._handleSave(value)
      else this.cancel()
    }
  }
