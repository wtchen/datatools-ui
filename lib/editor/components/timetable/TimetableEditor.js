  ...
  constructNewRow = (toClone: ?Trip = null, tripSeriesStartTime: ?number): ?Trip => {
    ...
    if (toClone) {
      ...
      let cumulativeTravelTime = !toClone
        ? 0
        : objectPath.get(newRow, 'stopTimes.0.arrivalTime') || objectPath.get(newRow, 'stopTimes.0.startPickupDropoffWindow')
      ...
      for (let i = 0; i < patternHalts.length; i++) {
        const halt = patternHalts[i]
        ...
        objectPath.set(newRow, `stopTimes.${i}.stopId`, patternHaltIsStop(halt) ? halt.stopId : patternHaltIsLocation(halt) ? halt.locationId : halt.areaId)

        // only set time if timepoint set to true or null
        // if (stop.timepoint === null || stop.timepoint) {
        if (patternHaltIsStop(halt)) {
          // $FlowFixMe Flow doesn't recognize our check for stopId property?
          cumulativeTravelTime += +halt.defaultTravelTime

          // $FlowFixMe Flow doesn't recognize our check for stopId property?
          objectPath.set(newRow, `stopTimes.${i}.stopId`, halt.stopId)
          objectPath.set(newRow, `stopTimes.${i}.arrivalTime`, cumulativeTravelTime)
          // $FlowFixMe Flow doesn't recognize our check for stopId property?
          cumulativeTravelTime += +halt.defaultDwellTime
          ...
          const haltId = getPatternHaltId(halt) // Handles check for Location vs. StopArea
        ...
        // }
        // if (stop.timepoint === null || stop.timepoint) {
        objectPath.set(newRow, `stopTimes.${i}.departureTime`, patternHaltIsStop(halt) ? cumulativeTravelTime : null)
        objectPath.set(newRow, `stopTimes.${i}.pickupType`, halt.pickupType)
        objectPath.set(newRow, `stopTimes.${i}.dropOffType`, halt.dropOffType)
        objectPath.set(newRow, `stopTimes.${i}.continuousPickup`, halt.continuousPickup)
        objectPath.set(newRow, `stopTimes.${i}.continuousDropOff`, halt.continuousDropOff)
        objectPath.set(newRow, `stopTimes.${i}.pickupBookingRuleId`, halt.pickupBookingRuleId)
        objectPath.set(newRow, `stopTimes.${i}.dropOffBookingRuleId`, halt.dropOffBookingRuleId)
        ...
        objectPath.set(newRow, `stopTimes.${i}.stopSequence`, halt.stopSequence || i)
        // }
      }
