  ...
  constructNewRow = (toClone: ?Trip = null) => {
    ...
    if (toClone) {
      ...
      for (let i = 0; i < patternHalts.length; i++) {
        ...
        objectPath.set(newRow, `stopTimes.${i}.stopId`, halt.hasOwnProperty('stopId') ? halt.stopId : halt.locationId)
        ...
        if (halt.hasOwnProperty('stopId')) {
          ...
          // $FlowFixMe Flow doesn't recognize our check for stopId property?
          cumulativeTravelTime += +halt.defaultTravelTimehalt.flexDefaultTravelTime
