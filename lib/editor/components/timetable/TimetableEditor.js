  ...
  constructNewRow = (toClone: ?Trip = null) => {
    ...
    if (toClone) {
      ...
      for (let i = 0; i < patternHalts.length; i++) {
        ...
        objectPath.set(newRow, `stopTimes.${i}.stopId`, patternHaltIsStop(halt) ? halt.stopId : halt.locationId)

        // only set time if timepoint set to true or null
        // if (stop.timepoint === null || stop.timepoint) {
        if (halt.hasOwnProperty('stopId')) {
          // $FlowFixMe Flow doesn't recognize our check for stopId property?
          ...
          // $FlowFixMe Flow doesn't recognize our check for stopId property?
          objectPath.set(newRow, `stopTimes.${i}.stopId`, halt.locationId)
