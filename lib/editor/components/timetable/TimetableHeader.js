...
export default class TimetableHeader extends Component<Props> {
  ...
  _onClickEdit = (e: SyntheticMouseEvent<HTMLInputElement>) => {
    const boundingRect = e.currentTarget.getBoundingClientRect()
    this.props.showTimetableContextMenu({
      context: 'edit-button',
      left: boundingRect.right - 330,
      top: boundingRect.bottom + 2
    })
  }
  ...
  _renderTimetableButtons = () => {
    ...
    const {edited, selected} = timetable
    const buttons = [{
      id: 'edit',
      tooltip: 'Edit stop times',
      props: {
  ...
  render () {
    ...
    const {hideDepartureTimes, trips} = timetable
    const calendars: Array<ServiceCalendar> = getTableById(tableData, 'calendar')
    const activeCalendar = calendars.find(c => c.service_id === activeScheduleId)
    const headerStyle = {
