...
export default class VirtualizedEntitySelect extends PureComponent<Props, State> {
  static defaultProps = {
    clearable: true,
    entityKey: 'id'
  }
  ...
  render () {
    ...
    // The filterOptions prop improves performance of the select component
    // dramatically. For more configuration options, see:
    // https://github.com/bvaughn/react-select-fast-filter-options#configuration-options
    // If shouldComponentUpdate is not turned on to check for changes in
    // length/selection, the createFilterOptions call should be commented out.
    // Otherwise, it will add signficant delay to the render acter each
    // updateActiveGtfsEntity action that is dispatched.
