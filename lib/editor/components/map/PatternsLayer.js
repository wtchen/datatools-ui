...
type Props = {
  ...
  constructControlPoint: (any) => void,
  ...
  route: GtfsRoute,
  setActiveEntity: (string, string, Entity, string, Pattern) => void,
  setActivePatternSegment: ?number => void,
  subEntityId: number,
  updateEditSetting: (string, any) => void
}

export default class PatternsLayer extends Component<Props> {
...
class PatternSegment extends Component<SegmentProps> {
  _onClick = (e: SyntheticInputEvent<HTMLInputElement> & {latlng: LatLng}) => {
    ...
    if (isEditing && segmentIsActive && editSettings.onMapClick === 'DRAG_HANDLES') {
      constructControlPoint({
        pattern,
        latlng: e.latlng,
        segmentIndex: index,
    ...
    if (!isEditing && patternIsActive) {
      updateEditSetting('editGeometry', true)
    }
    if (!patternIsActive) {
