...
import { addPolygonPoint } from '../../actions/map/locationsStrategies'
...
export default class EditorMap extends Component<Props, State> {
  ...
  _mapClicked = async (e: any) => {
    ...
    if (subComponent === 'trippattern' && editSettings.editGeometry) {
      ...
      // Start adding location points for a polygon
      switch (editSettings.onMapClick) {
        case 'NO_ACTION':
          break
        case 'ADD_POLYGON_POINT':
          return addPolygonPoint(e.latlng, true, null, activePattern)
      }
    }
  }
    ...
    switch (activeComponent) {
      ...
      case 'location':
        ...
        return (
          <LocationsLayer
            activeEntity={activeEntity}
            updateLocationShape={this.props.updateLocationShape}
          />
        )
      default:
        return null
