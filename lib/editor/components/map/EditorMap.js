...
import AddableLocationsLayer from './AddableLocationsLayer'
...
export default class EditorMap extends Component<Props, State> {
  ...
  _getMapComponents () {
    ...
    switch (activeComponent) {
        ...
        return (
          <FeatureGroup>
            ...
            <PatternStopsLayer
              ...
              activePatternStopAreas={this.props.activePatternStopAreas}
      ...
      case 'location':
        if (!activeEntity) return null
        const castedLocation = ((activeEntity: any): GtfsLocation)
        return (
          <LocationsLayer
            activeEntity={castedLocation}
            key={activeEntity.id}
            updatingLocationShape={this.props.updatingLocationShape}
          />
        )
      case 'stoparea':
        if (!activeEntity) return null
        const castedStopArea = ((activeEntity: any): GtfsStopArea)
        const idsToShow = castedStopArea.stop_id
        if (!idsToShow || idsToShow.length === 0) return null
        const stopAreaLocations = locations.filter(location => idsToShow.includes(location.location_id))
        const stopAreaStops = stops.filter(stop => idsToShow.includes(stop.stop_id))
        ...
        return (
          <FeatureGroup>
            <StopsLayer
              ...
              stops={stopAreaStops}
            />
            {stopAreaLocations.map(location => <LocationsLayer activeEntity={location} key={location.id} />)}
          </FeatureGroup>
        )
      default:
        return null
