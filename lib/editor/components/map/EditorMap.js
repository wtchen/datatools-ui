...
import {getMapLayers, constructStop, clickToLatLng, getFeedBounds} from '../../util/map'
...
export default class EditorMap extends Component<Props, State> {
  ...
  _mapBaseLayerChanged = (e: any) => {
    const layer = getMapLayers().find(l => l.name === e.name)
    if (!layer) return console.warn(`Could not locate map layer with name=${e.name}`, getMapLayers())
