...
import type {ControlPoint, Feed, GtfsStop, Pattern, PatternLocationGroup, PatternStop} from '../../../types'
import { mergePatternHaltsOfPattern } from '../../../gtfs/util'
import { patternHaltIsLocationGroup, patternHaltIsStop } from '../../util/location'
...
type Props = {
  ...
  patternStop: PatternStop | PatternLocationGroup,
...
export default class PatternStopMarker extends Component<Props> {
  ...
  render () {
    ...
    // Don't render a location
    if (!!patternStop && !patternHaltIsStop(patternStop) && !patternHaltIsLocationGroup(patternStop)) {
      return null
    }
    ...
    return (
      <Marker
        ref={`${patternHaltIsStop(patternStop) ? patternStop.id : stop.id}`}
        ...
        <Popup>
          <div // popup requires single child (i.e., single div)
            ...
            {!patternHaltIsStop(patternStop) && <h4>Location Group</h4>}
            <h5>{stopName}</h5>
            { // If we do not have a patternStop, do not render patternStop edit buttons
                  ...
                  <Row>
                    <Col xs={6}>
                      <FormGroup
                        ...
                        <MinuteSecondInput
                        // $FlowFixMe flow doesn't understand the patternHalt type check above
