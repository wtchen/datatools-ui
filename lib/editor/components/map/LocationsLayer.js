...
export default class LocationsLayer extends PureComponent<Props, State> {
  ...
  _onDeleted (e: L.LeafletEvent) {
    ...
    Object.values(layers).map(layer => {
      if (layer !== null && typeof layer === 'object' && layer.hasOwnProperty('_leaflet_id')) {
        ...
        if (
          ...
          const layerCoords = isPolygon ? coordSet[0].map(convertLatLngToArray) : coordSet.map(convertLatLngToArray)
    ...
    if (layerType !== null) this.props.updatingLocationShape({ geometryType: layerType, shapes: locationShapes })
  }
