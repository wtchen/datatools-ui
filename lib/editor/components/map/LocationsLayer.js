...
export default class LocationsLayer extends PureComponent<Props, State> {
  ...
  componentDidUpdate (prevProps: Props) {
    const locationShapes = this.props.activeEntity.location_shapes
    const prevLocationShapes = prevProps.activeEntity.location_shapes

    const mapToArray = ls => [ls.geometry_pt_lat, ls.geometry_pt_lon]

    const same = this.checkSameShape(locationShapes.map(mapToArray), prevLocationShapes.map(mapToArray))
    if (!same) this.setState({initialShapeIds: [...new Set(locationShapes.map(shapePt => shapePt.geometry_id))]})
  }
