...
export default class PatternLocationMarker extends Component<Props> {
  ...
  _onClick = () => {
    ...
    if (!this.props.patternLocation) return
  ...
  render () {
    ...
    // Don't render a stop
    if (!!patternLocation && patternLocation.hasOwnProperty('stop_id')) return null
      ...
      <FeatureGroup>
        {groupedLocationShapePts && Object.keys(groupedLocationShapePts).map(key =>
          <PolygonWithLabel
            ...
            key={patternLocation ? patternLocation.id : location.id}
            onClick={this._onClick}
            opacity={0.6}
            positions={groupedLocationShapePts[key]}
            ref={`${patternLocation ? patternLocation.id || patternLocation.locationId : location.id}`}
            tooltip={`${index + 1}`}
          ...
          >
            <Popup>
              <div // popup requires single child (i.e., single div)
                ...
                {!patternLocation && <h4>Location Group</h4>}
                <h5>{stopName}</h5>
                {patternLocation && <React.Fragment>
                  <Row>
                    <Col xs={12}>
                      <PatternStopButtons
