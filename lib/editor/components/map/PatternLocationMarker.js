...
import type {ControlPoint, Feed, GtfsLocation, Pattern, PatternLocation, PatternLocationGroup} from '../../../types'
import { mergePatternHaltsOfPattern } from '../../../gtfs/util'
import { groupLocationShapePoints, patternHaltIsLocation, patternHaltIsStop } from '../../util/location'
...
type Props = {
  ...
  patternLocation: PatternLocation | PatternLocationGroup,
...
export default class PatternLocationMarker extends Component<Props> {
  ...
  render () {
    ...
    // Don't render a stop
    if (!!patternLocation && patternHaltIsStop(patternLocation)) return null
      ...
      <FeatureGroup>
        {groupedLocationShapePts && Object.keys(groupedLocationShapePts).map(key =>
          <PolygonWithLabel
            ...
            key={patternHaltIsLocation(patternLocation) ? patternLocation.id : location.id}
            onClick={this._onClick}
            opacity={0.6}
            positions={groupedLocationShapePts[key]}
            // $FlowFixMe Flow doesn't understand patternHaltIsLocation
            ref={`${patternHaltIsLocation(patternLocation) ? patternLocation.id || patternLocation.locationId : location.id}`}
            tooltip={`${index + 1}`}
          ...
          >
            <Popup>
              <div // popup requires single child (i.e., single div)
                ...
                {!patternHaltIsLocation(patternLocation) && <h4>Location Group</h4>}
                <h5>{stopName}</h5>
                {patternHaltIsLocation(patternLocation) && <React.Fragment>
                  <Row>
                    <Col xs={12}>
                      <PatternStopButtons
