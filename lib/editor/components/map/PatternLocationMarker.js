...
export default class PatternLocationMarker extends Component<Props> {
  ...
  _onChangeTimeInLocation = (value: number) => {
    ...
    const { patternLocations } = activePattern
    patternLocations[index].flexDefaultZoneTime = value
    updatePatternStops(activePattern, [...activePattern.patternStops, ...patternLocations])
  }

  _onChangeTravelTime = (value: number) => {
    const {activePattern, index, updatePatternStops} = this.props
    const { patternLocations } = activePattern
    patternLocations[index].flexDefaultTravelTime = value
    updatePatternStops(activePattern, [...activePattern.patternStops, ...patternLocations])
  }
