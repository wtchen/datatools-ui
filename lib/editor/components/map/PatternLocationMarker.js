...
export default class PatternLocationMarker extends Component<Props> {
  ...
  _onChangeTimeInLocation = (value: number) => {
    ...
    const patternStops = [...activePattern.patternLocations]
    // Flex TODO: sort out flow error
    patternStops[index].flexDefaultZoneTime = value
    updatePatternStops(activePattern, [...activePattern.patternLocations, ...patternStops])
  }

  _onChangeTravelTime = (value: number) => {
    const {activePattern, index, updatePatternStops} = this.props
    const patternStops = [...activePattern.patternLocations]
    // Flex TODO: sort out these flow errors
    if (patternStops[index].default) return
    else if (patternStops[index].flexDefaultTravelTime) patternStops[index].flexDefaultTravelTime = value
    updatePatternStops(activePattern, [...activePattern.patternLocations, ...patternStops])
  }
...
// FLEX TODO: popup needs to be wired up properly
// the props are probably in the parent component, just need to make it down
// and then there might be some werid stuff with the flex math but should be fine
