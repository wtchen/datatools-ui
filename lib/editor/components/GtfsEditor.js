...
export default class GtfsEditor extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    const nextComponent = nextProps.activeComponent
    const nextTable = nextComponent && getTableById(nextProps.tableData, nextComponent, false)
    const componentChanged = nextComponent && nextComponent !== this.props.activeComponent
    if (componentChanged && !nextTable) {
    ...
    if (nextProps.activeComponent === 'route' && nextProps.feedSource && nextProps.activeEntity && (!this.props.activeEntity || nextProps.activeEntity.id !== this.props.activeEntity.id)) {
      // FIXME: add fetch trip patterns for GraphQL
