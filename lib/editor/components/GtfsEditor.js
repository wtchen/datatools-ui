...
export default class GtfsEditor extends Component<Props, State> {
  ...
  componentCleanUp = () => {
    ...
    stopLockTimer()
    removeEditorLockLastGasp(feedSourceId, editorActions.GTFS_EDITOR_LOCK)
  }
  ...
  onFocus = () => {
    ...
    if (namespace) {
      ...
      checkLockStatus(feedSourceId, editorActions.GTFS_EDITOR_LOCK)
    }
  }
  ...
  onVisibilityChange = () => {
    ...
    if (document.visibilityState === 'visible') {
      ...
      if (!errorStatus.modal && !!namespace) {
        lockEditorFeedSource(feedSourceId, editorActions.GTFS_EDITOR_LOCK)
      }
    } else {
