...
export default class EditorInput extends Component<Props> {
  ...
  _agencyToOption = (agency: GtfsAgency) => ({
    value: agency.agency_id,
    label: agency.agency_name,
    agency
  })

  _entityToOption = (entity: Entity, key: string) => {
    return entity
      ? {
        value: entity[key],
  ...
  render () {
    ...
    switch (field.inputType) {
      ...
      case 'GTFS_STOP': {
        ...
        if (!selectedStop) return console.warn(`Could not locate stop with ${key}=${currentValue}.`)
