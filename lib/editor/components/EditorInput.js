...
import type {GtfsAgency, GtfsLocation, GtfsStop, GtfsSpecField, Feed, Entity} from '../../types' // Add LocationGroupOption
...
import LocationGroupsEditor from './LocationGroupsEditor'
...
export default class EditorInput extends React.Component<Props> {
  ...
  render () {
    ...
    switch (field.inputType) {
      ...
      case 'GTFS_STOP_OR_LOCATION_LIST': {
        const stops: Array<GtfsStop> = getTableById(tableData, 'stop')
        const locations: Array<GtfsLocation> = getTableById(tableData, 'location')

        return (
          <FormGroup {...formProps}>
            <ControlLabel><small>Locations</small></ControlLabel>
