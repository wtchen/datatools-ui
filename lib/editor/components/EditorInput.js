...
import type {Entity, Feed, GtfsSpecField, GtfsAgency, GtfsStop} from '../../types' // Add LocationGroupOption
...
type Props = {
  ...
  field: GtfsSpecField,
  isNotValid: boolean,
  // locationGroupOptions: Array<LocationGroupOption>,
  onChange?: any => void,
...
export default class EditorInput extends React.Component<Props> {
  ...
  render () {
    ...
    switch (field.inputType) {
      ...
      case 'GTFS_STOP_OR_LOCATION': {
        const stops: Array<GtfsStop> = [...getTableById(tableData, 'stop')]
        const locations: Array<GtfsLocation> = [...getTableById(tableData, 'location')]

        return (
          <FormGroup {...formProps}>
            {basicLabel}
            <VirtualizedEntitySelect
              value={entityToOption(stops.find(s => s.stop_id === currentValue), 'stop_id') || entityToOption(locations.find(l => l.location_id === currentValue), 'location_id')}
              component={'stop or location'}
              entityKey={'stop_or_location_id'}
              entities={[ ...stops, ...locations.map(l => { return {...l, id: uuidv4()} }) ]}
              onChange={(change) => { this._processFieldChange(change.entity.stop_id || change.entity.location_id) }} />
          </FormGroup>
        )
      }
      default:
