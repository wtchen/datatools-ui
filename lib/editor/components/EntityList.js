...
import type {Sort} from '../reducers/data'
...
type Props = {
  ...
  deleteEntity: (string, string, number) => void,
  enterTimetableEditor: () => void,
  ...
  updateEntitySort: ({key: string}) => void,
  width: number
}
...
export default class EntityList extends Component<Props, State> {
  ...
  updateIndexes = (fromIndex: ?number, toIndex: ?number) => this.setState({fromIndex, toIndex})

  _getRow = ({index}: {index: number}) => this.props.list.get(index)

  _onChangeSort = (evt: SyntheticInputEvent<HTMLInputElement>) => this.props.updateEntitySort({key: evt.target.value})
