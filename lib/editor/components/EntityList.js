...
type Props = ContainerProps & {
  ...
  setInitialLocationShapeIds: typeof activeActions.setInitialLocationShapeIds,
  sort: DataStateSort,
...
export default class EntityList extends Component<Props, State> {
  ...
  _onRowClick = ({event, index}: {event: SyntheticKeyboardEvent<HTMLInputElement>, index: number}) => {
    ...
    const {activeComponent, activeEntity, feedSource, list, setActiveEntity, setInitialLocationShapeIds} = this.props
    ...
    if (shiftKey && activeEntity && !row.isActive) {
      ...
      setInitialLocationShapeIds(feedSource.id, activeComponent, row)
    }
    this.setState({fromIndex, toIndex})
  }
    ...
    const entityList = list.size
      ...
      : activeComponent === 'stop'
        ...
        : activeComponent === 'location'
          ? <div
            className='lead text-center'
            data-test-id='create-location-instructions'
            style={{marginTop: '10px', marginRight: '5px', marginLeft: '5px'}}
          >
            <small>Create a new location to begin flex area editing.</small>
          </div>
