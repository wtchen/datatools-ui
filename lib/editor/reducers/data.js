...
import type {FeedInfo, TripCounts} from '../../types'
...
const data = (state: DataState = defaultState, action: any): DataState => {
  switch (action.type) {
    ...
    case 'RESET_ACTIVE_GTFS_ENTITY': {
      ...
      const list = action.component === 'trippattern'
        ? stateEntity.tripPatterns
        : getTableById(state.tables, action.component, false)
      const entity = clone(list.find(e => e.id === action.entity.id))
      const {entityKey, editedKey} = getUpdateEntityKeys(action.component)
      return update(state, {
        active: {
