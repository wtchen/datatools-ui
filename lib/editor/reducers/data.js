...
import {ROUTE_STATUS_CODES} from '../util'
...
const data = (state: DataState = defaultState, action: Action): DataState => {
  switch (action.type) {
    ...
    case 'UPDATING_LOCATION_SHAPE': {
      const {geometryType, shapes} = action.payload
      return update(state, {
        active: {
          edited: {$set: true},
          entity: {
            geometry_type: {$set: geometryType},
            location_shapes: {$set: shapes}
          }
        }
      })
    }
    ...
    case 'UPDATE_ACTIVE_GTFS_ENTITY': {
      ...
      // When a route gets edited, set its status to "In Progress"
      const additionalProps = {}
      if (component === 'route' && !props.status) {
        additionalProps.status = ROUTE_STATUS_CODES.IN_PROGRESS
      }
      // FIXME: Check for pattern stops props (what about other sequenced lists)
      // and update stop sequence.
      return update(state, {
        active: {
          [entityKey]: {$merge: {...props, ...additionalProps}},
          [editedKey]: {$set: true}
        }
      })
    }
    ...
    case 'RECEIVE_GTFS_ENTITIES': {
      ...
      // stop_id field from the backend contains a comma separated list of stops and locations for the area
      if (data.feed.stop_areas && data.feed.stop_areas.length === 1 && data.feed.stop_areas[0].stop_id) {
        data.feed.stop_areas[0].stop_id = data.feed.stop_areas[0].stop_id.split(',')
      }
