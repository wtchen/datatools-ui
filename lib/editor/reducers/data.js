...
const defaultState = {
  ...
  lock: {
    ...
    // feedId: window.sessionStorage.getItem('feedId'),
...
const data = (state = defaultState, action) => {
  ...
  switch (action.type) {
    ...
    case 'SETTING_ACTIVE_GTFS_ENTITY': {
      ...
      if (feedSourceId !== state.active.feedSourceId) {
        stateUpdate.tables = {$set: {}}
      }
    ...
    case 'SET_EDITOR_CHECK_IN':
      // FIXME: Use some library to sync session storage with store (e.g.,
      // redux-sessionstorage)?
      // for (let key in action.payload) {
      //   if (action.payload.hasOwnProperty(key) && action.payload[key]) {
      //     window.sessionStorage.setItem(key, action.payload[key])
      //   } else {
