...
import {unflattenLocation} from '../util/location'
...
const data = (state: DataState = defaultState, action: Action): DataState => {
  switch (action.type) {
    ...
    case 'RECEIVE_BASE_GTFS': {
      ...
      const {locations} = feed

      if (locations) feed.locations = unflattenLocation(locations) || feed.locations
    ...
    case 'UPDATING_LOCATION_SHAPE': {
      ...
      return update(state, {
        active: {
          edited: {$set: true},
          entity: {
            location_shapes: {$set: shape}
          }
        }
      })
    }
    case 'SETTING_INITIAL_LOCATION_IDS': {
      const initialIds = action.payload
      return update(state, {
        active: {
          entity: {
            initial_location_shape_ids: {$set: initialIds}
          }
        }
      })
    }
    ...
    case 'SETTING_ACTIVE_GTFS_ENTITY': {
      ...
      const {activeEntity} = action.payload
      ...
      // hack to persist the initial location shape ids
      console.log('activeEntity', activeEntity)
      if (entity) entity.initial_location_shape_ids = Array.isArray(activeEntity.initial_location_shape_ids) ? [...activeEntity.initial_location_shape_ids] : []
      const active = {
        feedSourceId,
        entity,
        entityId,
    ...
    case 'RECEIVE_GTFS_ENTITIES': {
      let activePattern
      const {editor, component, initialLocationShapeIDs} = action.payload
      const {data} = action.payload
      const {feed: {locations}} = data
      ...
      if (locations) data.feed.locations = unflattenLocation(locations) || data.feed.locations
      ...
      if (component === 'location' && activeEntity) activeEntity.initial_location_shape_ids = initialLocationShapeIDs
