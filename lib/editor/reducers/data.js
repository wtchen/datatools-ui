...
const data = (state = defaultState, action) => {
  ...
  switch (type) {
    ...
    case 'SAVED_TRIP_PATTERN':
      action.tripPattern.patternStops.map(ps => ({...ps, id: generateUID()}))
      routeIndex = state.tables.route.findIndex(r => r.id === action.tripPatternrouteId)
      patternIndex = state.active.entity.tripPatterns.findIndex(p => p.id === action.tripPatternid)
      stateUpdate = {active: {}}

      // if pattern is active
      if (action.tripPatternid === state.active.subEntityId) {
        stateUpdate = {
          active: {
      ...
      } else if (action.tripPatternrouteId === state.active.entity.id) { // if pattern is entirely new
    ...
    case 'RECEIVE_TRIP_PATTERNS_FOR_ROUTE':
      ...
      console.log(tripPatterns[0].patternStops[0].id)
    ...
    case 'SET_ACTIVE_PATTERN_STOP':
      ...
      return update(state, {active: {patternStopId: {$set: action.payload.id}}})
    default:
      return state
  }
}
