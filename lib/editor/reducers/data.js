...
import {COMPONENT_LIST, getTableById, getKeyForId} from '../util/gtfs'
...
const data = (state = defaultState, action) => {
  let stateUpdate, key, subEntity, newState, routeIndex, stopIndex, patternIndex, scheduleExceptionIndex, activePattern
  ...
  switch (action.type) {
    case 'RECEIVE_BASE_GTFS':
      return update(state, {
        tables: {$set: action.payload.feed}
      })
    ...
    case 'SAVED_TRIP_PATTERN':
      ...
      if (patternIndex !== -1 && routeIndex !== -1) {
        ...
        stateUpdate.tables = {routes: {[routeIndex]: {tripPatterns: {[patternIndex]: {$set: pattern}}}}}
        ...
        return update(state, {
          tables: {routes: {[routeIndex]: {tripPatterns: {$push: [pattern]}}}},
    ...
    case 'RECEIVE_GTFS_ENTITIES': {
      ...
      if (!editor) {
        // Ignore entity fetches if not for the editor
        return state
      }
      ...
      // Response should only contain a single entity
      const activeEntity = entities[0]
      const activeIndex = activeTable
        ? activeTable.findIndex(e => e.id === activeEntity.id)
        : -1
      ...
      if (activeIndex !== -1) {
        stateUpdate = {
          tables: {[tableName]: {[activeIndex]: {$set: activeEntity}}},
          active: {
        ...
        console.warn(`No entity with id ${entities[0].id} found in list`, activeTable)
      }
      return state
    }
