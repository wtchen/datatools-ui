...
const data = (state: DataState = defaultState, action: Action): DataState => {
  switch (action.type) {
    ...
    case 'RECEIVE_BASE_GTFS': {
      ...
      if (!feed.locations) feed.locations = [] // TODO: DELETE ME, THIS IS A HACK TO ALLOW TEMP. SAVING OF FLEX TABLE

      // TODO: remove this hack once location data is able to be saved properly.
      let subData = feed.location_meta_data
      if (subData) {
        subData = subData.map(subDatum => {
          // Repackage
          const repackagedSubdatum = {
            'id': subDatum.id,
            'locationShapes': [],
            'location_id': subDatum.location_meta_data_id
          }
          // Unpackage properties string:
          // ~ is the backend key-value sep., # is the prop sep.
          if (subDatum.properties) {
            const propertiesFields = subDatum.properties.split('#').map(propPair => propPair.split('~'))
            // Package up the props from the properties string
            propertiesFields.forEach(propPair => { repackagedSubdatum[propPair[0]] = propPair[1] })
          }

          return repackagedSubdatum
        })
        feed.locations = subData
      }
    ...
    case 'UPDATING_LOCATION_SHAPE': {
      const shape = action.payload
      return update(state, {active:
        {entity:
          {locationShapes: {$set: shape}}
        }
      })
    }
    ...
    case 'RECEIVE_GTFS_ENTITIES': {
      let activePattern
      const {editor, component} = action.payload
      let {data} = action.payload

      // TODO: Fix this hack once location saving is properly set up
      // The backend uses "location_meta_data", the front end table is named for the spec, "locations"
      if (data.feed.location_meta_data) {
        const subData = data.feed.location_meta_data[0]

        // Unpackage properties string:
        // ~ is the backend key-value sep., # is the prop sep.
        const propertiesFields = subData.properties.split('#').map(propPair => propPair.split('~'))

        const locationData = {
          'id': subData.id,
          'locationShapes': [],
          'location_id': subData.location_meta_data_id
        }

        // Package up the props from the properties string
        propertiesFields.forEach(propPair => { locationData[propPair[0]] = propPair[1] })

        // Repackage
        data = {
          feed: {
            'feed_id': data.feed.feed_id,
            'feed_version': data.feed.feed_verison,
            'filename': data.feed.filename,
      ...
      if (component === 'pattern' && data.feed.shapes_as_polylines) {
      ...
      const entities = tableName ? clone(data.feed[tableName].map(mappingStrategy)) : null
      ...
      const activeEntity = entities ? entities[0] : null
      const activeTable = getTableById(state.tables, component, false)
