...
export const reducers = {
  'CONTROL_POINT_DRAG_START_OR_END' (state: EditSettingsState, action: any) {
    return update(state, {
      currentDragId: {$set: action.payload}
    })
  },
  'TOGGLE_PATTERN_EDITING' (state: EditSettingsState, action: any) {
    return {
      ...defaultState,
      editGeometry: action.payload
    }
  },
  'SETTING_ACTIVE_GTFS_ENTITY' (state: EditSettingsState, action: any) {
    return defaultState
  },
  'UPDATE_TEMP_PATTERN_GEOMETRY' (state: EditSettingsState, action: any) {
    return update(state, {
      controlPoints: {$set: action.payload.controlPoints},
      patternSegments: {$set: action.payload.patternSegments}
    })
  },
  'UPDATE_PATTERN_GEOMETRY' (state: EditSettingsState, action: any) {
    return update(state, {
      controlPoints: {$set: action.payload.controlPoints},
      patternSegments: {$set: action.payload.patternSegments}
    })
  },
  'UPDATE_EDIT_SETTING' (state: EditSettingsState, action: any) {
    return update(state, {
