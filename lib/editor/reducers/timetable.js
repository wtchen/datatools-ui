...
const timetable = (state: TimetableState = defaultState, action: Action): TimetableState => {
  switch (action.type) {
    case 'CLOSE_TIMETABLE_CONTEXT_MENU':
      return update(state, { contextMenu: { display: { $set: false }}})
    case 'CLEAR_GTFSEDITOR_CONTENT':
      return defaultState
    case 'SHOW_TIMETABLE_CONTEXT_MENU':
      const {context, left, top} = action.payload
      return update(state, {
        contextMenu: {
          $set: {
            context,
            display: true,
            // take the minimum left distance so menu isn't shown off the screen
            left: Math.min(window.innerWidth - 450, left - 50),
            top
          }
        }
      })
