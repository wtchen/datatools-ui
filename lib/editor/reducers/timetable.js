...
const timetable = (state = defaultState, action) => {
  let trips
  switch (action.type) {
    // case 'SET_ACTIVE_TRIP_PATTERN':
    //   const {subEntityId, subSubEntityId} = action.payload
    //   return update(state, {
    //     patternId: {$set: subEntityId},
    //     calendarId: {$set: subSubEntityId}
    //   })
    ...
    case 'RECEIVE_TRIPS_FOR_CALENDAR':
      trips = clone(sortAndFilterTrips(action.payload.trips, action.payload.pattern.useFrequency))
      return update(state, {
        trips: {$set: trips},
        status: {
          fetched: {$set: true},
