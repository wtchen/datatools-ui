...
import type UserPermissions, {
  Organization as DatatoolsOrganization,
  Permission as DatatoolsPermission,
  Project as DatatoolsProject
} from '../common/user/UserPermissions'
...
export type DatatoolsSettings = {
  ...
  account_type?: string,
  client_id: string,
  organizations: Array<DatatoolsOrganization>,
  permissions: Array<DatatoolsPermission>,
  projects: Array<DatatoolsProject>,
...
export type Feed = {
  ...
  dateCreated: number,
  deployable: boolean,
  deployedFeedVersionEndDate?: string,
  deployedFeedVersionId?: string,
  deployedFeedVersionStartDate?: string,
...
export type FeedVersionSummary = {
  ...
  retrievalMethod: ?RetrievalMethod,
  updated: number,
  // HACK: validationSummary is fully populated in FeedVersion.
  // Only startDate and endDate are available in FeedVersionSummary.
...
export type PatternLocationGroup = {
  continuousDropOff?: number,
  continuousPickup?: number,
  dropOffBookingRuleId?: ?string,
  dropOffType?: number,
...
export type PatternHalt = PatternStop | PatternLocation | PatternLocationGroup
...
export type Pattern = {|
  ...
  name: string,
  patternId: string,
  patternLocationGroups: Array<PatternLocationGroup>,
  patternLocations: Array<PatternLocation>,
...
// Spec fields are technically 'linestring', 'multilinestring', but react-leaflet-draw uses these
export type GeometryType = 'polygon' | 'polyline' | 'multipolygon' | 'multipolyline'
...
export type LeafletCoord = {
  lat: number,
  lng: number
}

export type LocationShapePoint = {
  geometry_id: string,
  geometry_pt_lat: number,
  geometry_pt_lon: number,
  id: number
}

export type LocationShape = Array<LocationShapePoint>

export type GtfsLocation = {|
  geometry_type: GeometryType,
  id: number,
  location_group_id?: string,
  location_group_name?: string,
  location_id: string,
  location_shapes: LocationShape,
  stop_desc?: string,
  stop_name?: string,
  stop_url?: string,
  zone_id?: string
...
export type ExceptionDate = string | 0 // 0's are included in the date type for new dates.
...
export type StopTime = {
  ...
  startPickupDropoffWindow?: number,
  stopHeadsign: string,
  stopId: string,
  stopSequence: number,
}
...
// FLEX TODO: Move to right spot
export type LocationGroupOption = {
  create?: boolean,
  label: any,
  value: string
}
...
export type MergeFeedsResult = {
  ...
  feedCount: number,
  mergeStrategy: 'DEFAULT' | 'CHECK_STOP_TIMES',
...
export type AccountType = {
  /** Only one account type should be default, otherwise one will be picked. */
  default?: boolean,
  name: string,
  /**
   * Whether to display any license terms when users of this account type first login.
   * If require_terms is true, a terms_url value must be provided.
   */
  require_terms?: boolean,
  /**
   * Optionally set terms_url to an absolute or relative to index.html
   * to display a link to the license terms for the account type.
   */
  terms_url?: string,
  type: string
}

export type AccountTypes = { [string]: AccountType }
