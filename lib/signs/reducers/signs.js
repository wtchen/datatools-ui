...
import type {SignsReducerState} from '../../types/reducers'
...
const signs = (
  state: SignsReducerState = defaultState,
  action: Action
): SignsReducerState => {
  let signs, entities
  switch (action.type) {
    ...
    case 'RECEIVED_GTFS_STOPS_AND_ROUTES':
      const {module, results} = action.payload
      if (module !== 'SIGNS') {
        return state
      }
      ...
      for (let i = 0; i < entities.length; i++) {
        const feed = results.feeds.find(f => f.feed_id === entities[i].entity.AgencyId)
        if (feed) {
    ...
    case 'RECEIVED_RTD_SIGNS':
      const {activeProject, rtdSigns, DisplayConfigurationDetails} = action.payload
      const entityList = []
      signs = rtdSigns
      for (var i = 0; i < signs.length; i++) {
        const action = signs[i]
        if (typeof action !== 'undefined' && DisplayConfigurationDetails && DisplayConfigurationDetails.length > 0) {
          for (var j = 0; j < DisplayConfigurationDetails.length; j++) {
            const ent = DisplayConfigurationDetails[j]
      ...
      const allSigns = rtdSigns.map((rtdSign) => {
