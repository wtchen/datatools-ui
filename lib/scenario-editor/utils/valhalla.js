 ...
 */
function handleGraphHopperRouting (path: any, individualLegs: boolean = false): any {
 ...
 */
export function handleMapzenRouting (json: any, individualLegs: boolean = false) {
  if (json && json.trip) {
    const legArray: Array<[number, number]> = json.trip.legs.map((leg, index) => {
    ...
    return individualLegs
      ? legArray
      : ([]: any).concat.apply([], legArray) // weird any notation to make flow happy
  } else {
    return null
  }
}
