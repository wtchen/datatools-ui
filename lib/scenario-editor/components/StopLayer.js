...
import { DomUtil, point, gridLayer } from 'leaflet'
...
export default class StopLayer extends GridLayer {
  ...
  createTile = (coords) => {
    ...
    // get the bounds
    const topLeft = map.unproject([coords.x * tileLength, coords.y * tileLength], coords.z)
    const brPoint = point([coords.x + 1, coords.y + 1])
    const botRight = map.unproject([brPoint.x * tileLength, brPoint.y * tileLength], coords.z)
