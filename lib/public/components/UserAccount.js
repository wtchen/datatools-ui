...
export default class UserAccount extends Component<Props> {
  ...
  render () {
    ...
    const accountType = userInfoForThisClient && userInfoForThisClient.account_type
    const accountTypeObject = accountTypes.find(acct => acct.type === accountType)
    ...
    const subscriptions: ?Array<Subscription> = userInfoForThisClient && userInfoForThisClient.subscriptions
