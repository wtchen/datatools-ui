...
import type {AccountType, Project} from '../../types'
...
export default class UserAccount extends Component<Props> {
  ...
  render () {
    ...
    const userInfoForThisClient = profile.app_metadata.datatools.find(
      dt => dt.client_id === process.env.AUTH0_CLIENT_ID
    )
    const defaultAccountType = accountTypes.find(acct => acct.default)
    const accountType = accountTypes.find(acct => acct.type === userInfoForThisClient.account_type)
    const accountTypeIsUnknown = userInfoForThisClient.account_type && !accountType
    const displayedAccountType = accountType
      ? accountType.name
      : accountTypeIsUnknown
        ? <span style={{ color: 'red' }}>Account type not configured ({userInfoForThisClient.account_type})</span>
        : (defaultAccountType && defaultAccountType.name)
    const accountTermsUrl = accountType && accountType.terms_url

    const subscriptions: Array<Subscription> = userInfoForThisClient.subscriptions
          ...
          <div>
            <Panel header={<h4>Profile information</h4>}>
              <ListGroup fill>
                ...
                {((accountTypes.length > 1 && displayedAccountType) || accountTypeIsUnknown) && (
                  <ListGroupItem>
                    ...
                    <div>
                      ...
                      {accountTermsUrl && (
                        <span>
                          {' - '}
                          <a href={accountTermsUrl} target='_blank'>Terms and conditions</a>
                        </span>
                      )}
                    </div>
                  </ListGroupItem>
