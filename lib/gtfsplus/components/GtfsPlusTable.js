...
import type {Entity, GtfsPlusTable as GtfsPlusTableType, Feed} from '../../types'

type Props = {
  currentPage: number,
  deleteRowClicked: (string, number) => void,
  feedSource: Feed,
  fieldEdited: ({fieldName: string, newValue: any, rowIndex: number, tableId: string}) => void,
  ...
  setVisibilityFilter: ({visibility: any}) => void,
...
class GtfsPlusRow extends Component<RowProps> {
  _onClickDeleteRow = (index: number) => {
