...
export default class GtfsPlusEditor extends Component<Props> {
  ...
  _gtfsEntitySelected = (type: string, entity: Entity) => {
    this.props.receiveGtfsEntities([entity])
  }

  _newRowsDisplayed = (rows: Array<any>) => {
    const {activeTableId, feedSource, feedVersionId, loadGtfsEntities, validation} = this.props
    const feedVersion = feedSource && feedSource.feedVersions && feedSource.feedVersions.find(v => v.id === feedVersionId)
