...
import type {GtfsRoute, GtfsStop, PatternLocation, PatternStop, User} from '../../types'
...
export function getGraphQLFieldsForEntity (type: string, editor: boolean = false) {
  ...
  // const locationShapeFields = `location_shapes (limit -1) {
  //   id
  //   shape_id
  //   shape_polygon_id
  //   shape_ring_id
  ...
  switch (type.toLowerCase()) {
    case 'location':
      // Because of the geoJSON structure of locations,
      // the location properties are packaged into the format the backend requires
      // TODO: override manual creation of fields
      return `
        id
        location_meta_data_id
        properties
...
): Array<PatternStop | PatternLocation> {
  return [...patternLocations, ...patternStops]
