...
import type {GtfsRoute, GtfsStop, Pattern, PatternLocation, PatternLocationGroup, PatternStop, User} from '../../types'
...
export function getGraphQLFieldsForEntity (type: string, editor: boolean = false) {
  ...
  const patternLocationFields = `pattern_locations (limit: -1) {
    id
    location_id
    stop_sequence
    pickup_type
    ...
    case 'route':
      return `
        ...
        tripPatterns: patterns (limit: -1) {
          ...
          name
          ${patternStopFields}
          ${patternLocationFields}
          ${patternLocationGroupFields}
          ${shapeFields}
        }
      `
    default:
...
/**
 * This helper method merges different types of pattern halts into one unified array
 * sorted by stop sequence
 */
export function mergePatternHalts (
  patternStops: Array<PatternStop>,
  patternLocations: Array<PatternLocation>,
  patternLocationGroups: Array<PatternLocationGroup>
): Array<PatternStop | PatternLocation | PatternLocationGroup> {
  return [...patternLocations, ...patternLocationGroups, ...patternStops]
    .sort((a, b) => a.stopSequence - b.stopSequence)
}
export function mergePatternHaltsOfPattern (pattern: Pattern) {
  return mergePatternHalts(pattern.patternStops, pattern.patternLocations, pattern.patternLocationGroups)
}
