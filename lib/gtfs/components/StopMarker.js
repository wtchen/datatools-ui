...
type Props = {
  newEntityId: ?number,
  onStopClick: ?(props: {feed: Feed, entities: Array<GtfsStop>}) => void,
  popupAction: ?string,
...
export default class StopMarker extends Component<Props> {
  ...
  _getChildStopsContent = () => {
    const {popupAction, stop} = this.props
    ...
    return (
      <div className='well' style={{marginTop: '5px'}}>
        ...
        <Button
          ...
          <Icon type='map-marker' /> {popupAction} all stops
        </Button>
      </div>
    )
  }
          ...
          <div>
            ...
            {onStopClick && (
              <Button
                ...
                <Icon type='map-marker' /> {popupAction}{' '}
                {hasChildStops
                  ? 'this stop only'
                  : 'stop'
                }
