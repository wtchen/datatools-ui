...
import type {AppState, Feed, GtfsRoute, GtfsStop} from '../../types'
import type {UserState} from '../../manager/reducers/user'
import type {MapState} from '../reducers/filter'
...
class GtfsMapSearch extends Component<Props, State> {
  ...
  getPatternsForRoute (feed: Feed, route: GtfsRoute) {
    ...
    if (!token) {
      console.warn('Cannot fetch patterns without user token', token)
      return
    }
    ...
    return fetch(`${GTFS_GRAPHQL_PREFIX}`, {
      ...
      .then(json => {
        ...
        const result: {pattern_id: string, feed: Feed, route: GtfsRoute} = {
          ...pattern,
          route,
          feed
        }
  ...
  render () {
    ...
    const zoomMessage = mapState.zoom <= 13
