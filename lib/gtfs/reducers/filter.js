...
const gtfsFilter = (state: FilterState = defaultState, action: any): FilterState => {
  switch (action.type) {
    ...
    case 'ADD_ACTIVE_FEED':
      return update(state, {activeFeeds: {[action.payload.id]: {$set: true}}})
    case 'REMOVE_ACTIVE_FEED':
      return update(state, {activeFeeds: {[action.payload.id]: {$set: false}}})
