...
export function secureFetch (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    const body = payload && isJSON
      ...
      : payload
    const basicError = `Error making ${method} request to ${url}`
    return fetch(url, {method, headers, body})
      // Catch basic error during fetch
      .catch(err => {
        console.error(basicError, err)
        return dispatch(setErrorMessage({basicError}))
      })
      .then(res => dispatch(handleResponse(method, url, res, raw, actionOnFail, basicError)))
  }
}

function handleResponse (method, url, res, raw, actionOnFail, basicError) {
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    const errorMessage = getErrorMessageFromJson(json, status, method, url, actionOnFail, basicError)
...
function getErrorMessageFromJson (
  ...
  if (json) {
    ...
    if (status >= 500) {
      detail = detail ? detail + networkError : networkError
    }
  }
  console.error(basicError, errorMessage)
