...
function getErrorMessage (status, method, url) {
  let errorMessage = `Error (${status}) making ${method} request to ${url}`
  if (status >= 500) {
...
export function secureFetch (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return fetch(url, {method, headers, body})
      ...
      .catch(err => {
        const message = getErrorMessage(status, method, url)
...
function getErrorMessageFromJson (
  ...
  let action = 'RELOAD'
  let detail
  let errorMessage = getErrorMessage(status, method, url)
  if (status < 500) {
    action = status === 401
      ? 'LOG_IN'
      : actionOnFail
