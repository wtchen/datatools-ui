...
type DatatoolsApps = Array<DatatoolsJson>
...
export default class UserPermissions {
  ...
  constructor (datatoolsApps: DatatoolsApps) {
    ...
    const datatoolsJson: DatatoolsJson = datatoolsApps && datatoolsApps.constructor === Array
      ? datatoolsApps.find(dt => dt.client_id === process.env.AUTH0_CLIENT_ID)
      : datatoolsApps
    if (datatoolsJson) {
  ...
  isOrganizationAdmin (orgId: ?string = null) {
    if (this.isApplicationAdmin()) return true
  ...
  getOrganizationPermissions (organizationId: string) {
    if (!this.hasOrganization(organizationId)) return null
    return this.organizationLookup[organizationId].permissions
  }

  hasProject (projectId: string, organizationId: ?string = null) {
  ...
  getProjectPermission (projectId: string, permissionType: string): string {
    if (!this.hasProject(projectId)) return null
