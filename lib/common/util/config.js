...
export function getGtfsSpecField (tableId: string, fieldName: string): any {
  const specTable = getGtfsSpec().find(t => t.id === tableId)
  if (!specTable) {
    throw new Error("Tried to access field of table that doesn't exist!")
  }

  return specTable.fields.find(field => field.name === fieldName)
}
