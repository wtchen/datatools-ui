 ...
 */
function showLicenseIfNeeded (accountTypes: Array<AccountType>, profile: UserProfile) {
  const userSettings = getSettingsFromProfile(profile)
  if (userSettings) {
    const accountType = accountTypes.find(acct => acct.type === userSettings.account_type)
    if (
      accountType &&
