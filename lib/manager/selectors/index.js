...
export const getTimetableData: AppState => {
  ...
  (timetables, showArrivals, timepointFilter) => {
    ...
    const STOP_COL_WIDTH = showArrivals ? 150 : 75
    ...
    const rows = pattern.trips.map((trip, tripIndex) => {
      ...
      trip.stop_times.forEach((stopTime, index) => {
        ...
        row[`${index}-dwell`] = departureTime - arrivalTime === 0
          ? '--'
          : `${Math.floor((departureTime - arrivalTime) / 60 * 100) / 100} min.`
      })
      return row
    })
    ...
    rows
      ...
      .forEach((row, rowIndex) => {
        if (rowIndex > 0) {
          // For first departure, record times between trips.
    ...
    firstTrip.stop_times.forEach((stopTime, i) => {
      ...
      const dataFormat = (cell, row, extraData, rowIndex) => {
        ...
        // rowIndex === 1 && console.log(columnData, extraData, diff, count)
        // If the cell matches the most frequent travel time, make color
        // black. Otherwise, scale lightness by relative frequency.
        const lightPercent = frequencyIndex === columnData.length - 1
          ? 0
          : ((rows.length - count) / rows.length) * 40
        const color = travelTimeIndex < mostFrequentIndex
          ? '102' // green: for travel times that are less than average (i.e., quicker)
          : travelTimeIndex > mostFrequentIndex
            ? '0' // red: for travel times that are longer than average (i.e., slower)
            : '200' // blue: blue should never actually appear because the light should be zero in this case (black)
        const textStyle = {color: `hsl(${color}, 100%, ${lightPercent.toString()}%)`}
        return <span style={textStyle}>{secondsAfterMidnightToHHMM(cell)}</span>
      }
