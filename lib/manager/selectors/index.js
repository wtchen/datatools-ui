...
import type {AppState, TimetableStop, ValidationStop} from '../../types/reducers'
  ...
  patterns => {
    return patterns.map(pattern => {
      ...
      pattern.trips.forEach(trip => {
        fillStartTimesForTrip(trip, tripsPerHour)
      })
      return {
...
/**
 * Helper function to fill tripsPerHour array with counts based on a single trip.
 */
function fillStartTimesForTrip (trip, tripsPerHour: Array<number>) {
  const startTimes = []
