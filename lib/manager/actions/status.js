...
const setServerInfo = createAction(
  'SET_SERVER_INFO',
  (payload: {
    commit: string,
    repoUrl: string
...
export function fetchServerInfo () {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (!getState().status.serverInfo) {
      ...
      return dispatch(secureFetch('/api/manager/public/serverinfo'))
        .then(response => response.json())
        .then(info => {
          if (info.commit && info.repoUrl) {
            ...
            throw new Error('unable to interpret server info')
          }
        })
        .catch(() => {
