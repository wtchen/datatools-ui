...
function getMergeFeedModalContent (result: MergeFeedsResult): ModalContent {
  ...
  if (failed) {
    if (tripIdsToCheck && tripIdsToCheck.length > 0) {
      const tripIdListItems = []
      tripIdsToCheck.map(id => {
        tripIdListItems.push(<li key={id.toString()}>{id}</li>)
      })
    ...
    if (Object.keys(remappedIds).length > 0) {
      ...
      for (const key in remappedIds) {
        ...
        remappedIdStrings.push(
          <li key={key}>{tableAndId.join(':')} -> {remappedIds[key]}</li>
        )
      }
    ...
    if (skippedIds.length > 0) {
      ...
      for (const key in skippedRecordsForTables) {
        ...
        skippedRecordsStrings.push(<li key={key}>{tableItem}</li>)
      }
