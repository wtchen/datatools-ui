 ...
 */
export function createFeedSource (properties: {name: string, projectId: string }) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(url, 'post', properties))
      .then((res) => res.json())
      .then((createdFeed) => {
        return dispatch(fetchProjectWithFeeds(properties.projectId)).then(() => {
          browserHistory.push(`/feed/${createdFeed.id}`)
        })
      })
      .then(feedSources => dispatch(receiveFeedSources({
        feedSources,
