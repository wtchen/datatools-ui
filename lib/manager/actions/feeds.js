...
export function deleteFeedSource (feedSource: Feed) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (feedSource.isCreating) return dispatch(fetchProjectFeeds(feedSource.projectId, false, true))
    return dispatch(secureFetch(`${FS_URL}/${feedSource.id}`, 'delete'))
      .then(res => dispatch(fetchProjectFeeds(feedSource.projectId, false, true)))
  }
}
