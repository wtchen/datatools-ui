  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(`${FS_URL}/${feedSourceId}`))
      ...
      .then(feedSource => {
        ...
        if (shouldFetchRelatedTables) dispatch(fetchRelatedTables(feedSource))
        return feedSource
      })
  }
}
...
export function onFeedSourceViewerMount (initialProps: any) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (!project) {
      ...
      // Fetch feed source with versions and snapshots if the project is not
      // found. Note: We avoid handling other cases (e.g., fetching only feed
      // versions if the feed source has already been fetched) because this can
      // lead to difficulties troubleshooting the waterfall of fetches, especially
      // when the number of records (e.g., for feed versions) grows large. And,
      // repeating a fetch for one feed source record is relatively small
      // potatoes when compared with fetching 90+ feed versions.
      dispatch(fetchFeedSource(feedSourceId, true))
    } else {
