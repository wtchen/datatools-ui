    ...
    return dispatch(secureFetch(url, 'put', changes))
      .then((res) => {
        if (res.status >= 400) {
          ...
          dispatch(setErrorMessage({message: 'Error updating feed source.'}))
        }
...
export function deleteFeedSource (feedSource, changes) {
  return function (dispatch, getState) {
    ...
    return dispatch(secureFetch(url, 'delete'))
      .then((res) => {
        // if (res.status >= 400) {
        //   return dispatch(setErrorMessage({message: 'Error deleting feed source'}))
        // }
        return dispatch(fetchProjectFeeds(feedSource.projectId))
      })
  }
...
export function fetchFeedSource (feedSourceId, withVersions = false, withSnapshots = false) {
  return function (dispatch, getState) {
    ...
    return dispatch(secureFetch(url))
      .then(res => {
        if (res.status >= 400) {
          // dispatch(setErrorMessage({message: 'Error getting feed source'}))
    ...
    return dispatch(secureFetch(url, 'post'))
      .then(res => {
        if (res.status === 304) {
          ...
          dispatch(setErrorMessage({message: 'Error fetching feed source'}))
        } else {
