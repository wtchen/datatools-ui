...
export function fetchProjectWithFeeds (projectId: string, unsecure: ?boolean) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    // Don't fetch project or feeds if project and feeds are already present and the feed sources and projects aren't being actively saved
    const state = getState()
    const project = state.projects.all.find(p => p.id === projectId)
    const isSaving = state.status.message && state.status.saving
    if (!isSaving && project && project.feedSources && project.feedSourceCount && project.feedSources.length <= project.feedSourceCount) {
      return
    }
