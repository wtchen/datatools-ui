  ...
  return function (dispatch, getState) {
    ...
    return secureFetch('/api/manager/secure/project', getState())
      ...
      .then(projects => {
        ...
        if (getActive) {
          let activeProject = getActiveProject(getState())
          if (!activeProject) {
            activeProject = projects.find(proj => proj.id === getConfigProperty('application.active_project')) ||
              projects[0]
            return dispatch(setActiveProject(activeProject))
            .then(() => {
