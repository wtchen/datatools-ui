...
import {fetchFeedVersionIfNotExistsInFeedSource} from './versions'
...
export function calculateFeedSourceTableComparisonColumn (projectId: string) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (feedSourceTableFilterCountStrategy === 'PUBLISHED') {
      ...
      dispatch(fetchPublishedComparisonVersionsForProject(project))
    } else if (
      project &&
      ...
      // PUBLISHED
      dispatch(setFeedSourceTableComparisonColumn('DEPLOYED'))
      ...
      // compare to
      dispatch(setFeedSourceTableComparisonColumn(null))
    }
  }
}

/**
 * Fetch all versions needed to properly calculate the values in the comparison
 * column when comparing to the last published version.
 */
function fetchPublishedComparisonVersionsForProject (project: ?Project) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    dispatch(setFeedSourceTableComparisonColumn('PUBLISHED'))
    if (!project || !project.feedSources) return

    project.feedSources.forEach(feedSource => {
      if (feedSource.publishedVersionId) {
        dispatch(fetchFeedVersionIfNotExistsInFeedSource(
          feedSource,
          feedSource.publishedVersionId
        ))
      }
    })
  }
