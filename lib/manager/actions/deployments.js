...
import type {Deployment, Feed, FeedVersion, UserProfile} from '../../types'
...
export function addFeedVersion (deployment: Deployment, feedVersion: $Shape<FeedVersion>) {
...
export function refreshDeploymentData (
  deployment: ?Deployment,
  profile: ?UserProfile
) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    if (profile && deployment) {
      dispatch(fetchDeploymentAndProject(deployment.id))
        .then(deployment => dispatch(fetchProjectFeeds(deployment.project.id)))
    }
  }
}
...
export function updateDeployment (deployment: Deployment, changes: $Shape<Deployment>) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    const {id} = deployment
    dispatch(savingDeployment())
