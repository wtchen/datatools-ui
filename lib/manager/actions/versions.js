...
export const receiveGTFSEntities = createAction(
  ...
  (payload: {
    ...
    editor: ?boolean,
    id: ?string | number,
    initialLocationShapeIDs: Array<string>,
    namespace: string,
    replaceNew: ?boolean
  }) => payload
)
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(fetchGraphQL({query, variables}))
      .then(data => {
        ...
        const initialLocationShapeIDs = state.editor.data.active.entity.initial_location_shape_ids || []
        dispatch(receiveGTFSEntities({namespace, id, component: type, data, editor, replaceNew, initialLocationShapeIDs}))
        if (editor) {
