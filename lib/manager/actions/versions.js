...
export function fetchGTFSEntities ({namespace, id, type, editor = false, replaceNew = false}) {
  return function (dispatch, getState) {
    ...
    return dispatch(fetchGraphQL({
      ...
      .then(data => {
        ...
        if (replaceNew) {
          ...
          console.log('I hope active entity gets set automatically')
          ...
          // entity. Also, updating trip patterns or trips does not come through
          // this code path, so we don't need to handle those longer URLs.
          browserHistory.push(`/feed/${feedSourceId}/edit/${type}/${id}`)
        }
      })
  ...
  return function (dispatch, getState) {
    ...
    .then(credentials => {
      if (getConfigProperty('application.data.use_s3_storage')) {
        // Download via s3 if feed is stored there.
...
/**
 * Download some object at the provided folder/filename (aka key) using the
 * server-provided temporary s3 credentials.
 * @param  {[type]} credentials   [description]
 * @param  {[type]} filename      [description]
 * @param  {[type]} [folder=null] [description]
