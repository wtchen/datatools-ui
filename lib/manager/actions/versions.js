...
/**
 * Fetches a feed version only if the feed version doesn't already exist in the
 * given feed source
 */
export function fetchFeedVersionIfNotExistsInFeedSource (
  feed: Feed,
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    if (
      !feed.feedVersions ||
        !feed.feedVersions.some(version => version.id === feedVersionId)
    ) {
