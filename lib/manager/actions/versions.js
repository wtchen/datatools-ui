...
// import {getControlPoints} from '../../editor/selectors'
    ...
    return uploadFile({file, url, token: getState().user.token})
      .then(res => {
        ...
        if (res.status === 304) {
          ...
          dispatch(setErrorMessage({message: 'Error uploading feed source'}))
        } else {
...
const requestingValidationResult = createAction('REQUESTING_VALIDATION_RESULT')
const receiveValidationResult = createAction('RECEIVE_VALIDATION_RESULT')
...
export function fetchGTFSEntities ({namespace, id, type, editor = false}) {
  return function (dispatch, getState) {
    // If fetching for the editor, query on csv_line
    ...
    const query = `
      query entityQuery($namespace: String, $${entityIdField}: ${editor ? 'Int' : '[String]'}) {
    ...
    return dispatch(fetchGraphQL({query, variables: {namespace, [entityIdField]: editor ? +id : id}}))
      .then(res => res.json())
      .then(data => dispatch(receiveGTFSEntities({namespace, id, component: type, data, editor})))
      .then(() => {
        if (type === 'route') {
          // dispatch(setControlPoints(getControlPoints(getState())))
          // FIXME: do some of these things on fetch for trip pattern?
          // const stops = getTableById(getState().editor.data.tables, 'stop')
          ...
          // const activePatternStops = getStopsForPattern(activePattern, stops)
        }
      })
...
export function fetchValidationErrors ({feedVersion, errorType, limit, offset}) {
  return function (dispatch, getState) {
    ...
    return dispatch(fetchGraphQL({query, variables: {namespace, errorType: [errorType], limit, offset}}))
      .then(res => res.json())
      .then(result => {
        if (result.feed) {
...
export function fetchValidationResult (feedVersion, isPublic) {
  return function (dispatch, getState) {
    ...
    return dispatch(fetchGraphQL({query, variables: {namespace}}))
      ...
      .then(result => {
        if (result.feed) {
          dispatch(receiveValidationResult({feedVersion, validationResult: result.feed}))
        }
      })
      .catch(err => console.log(err))
  }
...
export function downloadS3Key (credentials, filename, folder = null) {
  return function (dispatch, getState) {
    ...
    s3.getObject({Bucket, Key}, (err, data) => {
      if (err) {
        console.log(err)
        dispatch(setErrorMessage({message: `Error downloading feed:\n${err}`}))
      }
      if (data) {
