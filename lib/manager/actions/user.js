...
import { getUserInfoForThisClient, isSettingForThisClient } from '../../common/util/user'
...
export function updateTargetForSubscription (profile: UserProfile, target: string, subscriptionType: string) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    const userInfo = getUserInfoForThisClient(profile.app_metadata.datatools)
    const subscriptions = (userInfo && userInfo.subscriptions) ||
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(url, 'put', payload))
      ...
      .then(responseJson => {
        ...
        if (responseJson.user_id === user.user_id) {
