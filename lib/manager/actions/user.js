...
export function updateUserMetadata (profile, props) {
  return function (dispatch, getState) {
    const {user_metadata: userMetadata} = profile
    const CLIENT_ID = process.env.AUTH0_CLIENT_ID
    const hasDatatoolsEntry = userMetadata && userMetadata.datatools
    const updatedMetadata = hasDatatoolsEntry
      ? userMetadata
      : { lang: 'en', datatools: [] }
    if (updatedMetadata.datatools.findIndex(d => d.client_id === CLIENT_ID) === -1) {
      // User metadata exists but does not have a DT entry for this CLIENT_ID
      updatedMetadata.datatools.push({ client_id: CLIENT_ID })
    }
    const clientIndex = updatedMetadata.datatools.findIndex(d => d.client_id === CLIENT_ID)
