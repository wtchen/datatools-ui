...
// FIXME: do we still need this action?
...
const voidAction = createVoidPayloadAction('VOID_ACTION')
 ...
 * login.
 */
export function login0 (redirectOnSuccess: ?string) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
 ...
 * login.
 */
export function login (auth0: Auth0ContextInterface, redirectOnSuccess: ?string) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    // dispatch(setRedirectOnLogin(redirectOnSuccess))
    // Login component exists at login route.
    // browserHistory.push('/login')
    ...
    auth0.loginWithRedirect({
      redirectUri: `${window.location.origin}/login?to=${redirectOnSuccess || '/home'}`
    })
  }
}
 ...
 */
export function logout0 (logoutFromAuth0: ?boolean = true) {
  if (logoutFromAuth0) {
    auth0.logout()
  }
  return userLogout()
...
export function receiveTokenAndProfile (authResult: ?{profile: UserProfile, token: string}) {
  ...
  // FIXME: is this call still necessary?
  if (!authResult) {
    return voidAction()
  }
