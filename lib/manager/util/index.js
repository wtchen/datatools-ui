...
import type {
  FeedSourceTableFilterStrategies,
...
export const feedFilterOptions: {
  [FeedSourceTableFilterStrategies]: (Feed) => boolean
} = {
  'all': () => true,
  ...
  'expiring-within-5-days': (feed) => !!feed.latestValidation &&
    moment(feed.latestValidation.endDate).subtract(5, 'days').isSameOrBefore(moment()) &&
    moment(feed.latestValidation.endDate).isSameOrAfter(moment()),
  ...
  'no-version': (feed) => !feed.latestValidation
}
...
export function getFeedFilterCountsForProject (project: Project): {
  [FeedSourceTableFilterStrategies]: number
} {
