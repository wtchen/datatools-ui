...
import {getVersionByFilterStrategy} from './version'
...
export function filterFeedByFilterCountStrategy (
  ...
  switch (filterCountStrategy) {
    ...
    case 'DEPLOYED':
    case 'PUBLISHED':
      const version = getVersionByFilterStrategy(
        project,
        feed,
        filterCountStrategy
      )
      if (version) {
        // The version can be a FeedVersion or SummarizedFeedVersion type
        // The SummarizedFeedVersion validationResult is actually of the type
        // ValidationSummary. Therefore, check to make sure the summary is
        // being used if it exists which indicates that we're working with
        // a FeedVersion.
        if (version.validationSummary) {
          data = version.validationSummary
        } else {
          data = version.validationResult
        }
      } else {
        return false
      }
