...
import {feedFilterOptions, feedSortOptions} from '../'
...
function setTestTime (time: number) {
...
describe('manager > util > index >', () => {
  describe('feedFilterOptions', () => {
    ...
    afterEach(() => {
      // restore standard functionality of Date library after each test if the
    ...
    const filterTestCaseData = {
      ...
      'no-version': {
        expected: ['test feed with no version']
      }
    }

    Object.keys(feedFilterOptions).forEach(optionKey => {
      ...
      it(`should filter feeds with '${optionKey}' strategy for date ${moment(filterOptionTestDate).toISOString()}`, () => {
        ...
        const filtered = clone(mockFeedsForSortTest)
          .filter(feedFilterOptions[optionKey])
          .map(getFeedName)
