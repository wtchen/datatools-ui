...
export default class GeneralSettings extends Component<Props, State> {
  ...
  _onToggleFlex = () => {
    const {feedSource, updateFeedSource} = this.props
    updateFeedSource(feedSource, {flex: !feedSource.flex})
  }
  ...
  render () {
    ...
    return (
      <Col xs={7}>
        ...
        <Panel header={<h3>Flex Service</h3>}>
          <FormGroup>
            <Checkbox
              checked={feedSource.flex}
              onChange={this._onToggleFlex}>
              <strong>Feed includes flex service</strong>
            </Checkbox>
            <small>Enable this feed source to include flex service.  Flex service describes operations that follow a schedule, but also include flexible features such as dial-a-ride service, route deviation, continuous stops, or point to zone service.</small>
          </FormGroup>
        </Panel>
