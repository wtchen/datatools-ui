...
export default class GeneralSettings extends Component<Props, State> {
  ...
  _onToggleFlex = () => {
    const {feedSource, updateFeedSource} = this.props
    updateFeedSource(feedSource, {flexUIFeaturesEnabled: !feedSource.flexUIFeaturesEnabled})
  }
  ...
  render () {
    ...
    return (
      <Col xs={7}>
          ...
          <ListGroup fill>
            ...
            <ListGroupItem>
              <FormGroup>
                ...
                <small>
                  Enable this feed source to be deployed to an OpenTripPlanner
                  (OTP) instance (defined in organization settings) as part of a
                  collection of feed sources or individually.
                </small>
              </FormGroup>
            </ListGroupItem>
          </ListGroup>
        ...
        <Panel>
          <Panel.Heading>{this.messages('flex.title')}</Panel.Heading>
          <ListGroup fill>
            <ListGroupItem>
              <FormGroup>
                <Checkbox
                  checked={feedSource.flexUIFeaturesEnabled}
                  disabled={feedSource.flex}
                  onChange={this._onToggleFlex}
                >
                  <strong>{this.messages('flex.checkbox')}</strong>
                </Checkbox>
                <small>{this.messages('flex.description')}</small>
                {feedSource.flex && (
                  <small style={{ display: 'block', paddingTop: 5 }}>
                    Flex tools in the editor can not be disabled while flex
                    features are used in the feed.
                    <strong>
                      To disable flex tools in the editor, remove all flex
                      features from the feed. This checkbox will then be
                      un-checkable.
                    </strong>
                  </small>
                )}
        ...
        <Panel>
          <Panel.Heading>Automatic Fetch</Panel.Heading>
          <ListGroup fill>
            <ListGroupItem>
              <FormGroup>
            ...
            <ListGroupItem>
              <FormGroup>
                ...
                <small>
                  Set this feed source to fetch automatically. (Feed source URL
                  must be specified and project auto fetch must be enabled.)
                </small>
              </FormGroup>
              {autoFetchFeed ? (
                <FeedFetchFrequency
        ...
        <Panel>
          <Panel.Heading>Labels</Panel.Heading>
          <Panel.Body>
        ...
        <Panel bsStyle='danger'>
          ...
          <ListGroup fill>
            <ListGroupItem>
              ...
              <h4>
                Make this feed source{' '}
                {feedSource.isPublic ? 'private' : 'public'}.
              </h4>
              <p>
                This feed source is currently{' '}
