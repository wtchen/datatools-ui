...
import { Radio, FormGroup, InputGroup, HelpBlock, ListGroup, ListGroupItem, Row, Col, Button, Panel, FormControl, Glyphicon, Badge, ButtonGroup, DropdownButton, MenuItem, Label } from 'react-bootstrap'
...
export default class DeploymentViewer extends Component {
  ...
  _onChangeSearch = evt => this.setState({searchText: evt.target.value})
  ...
  _onSelectTarget = target => this.setState({target})
  ...
  render () {
    ...
    const isWatchingDeployment = user.subscriptions.hasFeedSubscription(
      project.id,
      deployment.id,
      'deployment-updated'
    )
    ...
    const oldDeployment = target && deployments.find(d => d.deployedTo === target && deployment.routerId === d.routerId)
    return (
      <div>
        {target &&
              ...
              <ListGroup fill>
                ...
                <ListGroupItem>
                  <Row>
                    <Col xs={6}>
                      <h4>
                        <Glyphicon glyph='list' /> {getMessage(messages, 'versions')}{' '}
                        <Badge>{versions.length}</Badge>
                        <br />
                        <small>
                          <Glyphicon glyph='calendar' />{' '}
                          {earliestDate} {getMessage(messages, 'to')} {latestDate}
                        </small>
                      </h4>
                    </Col>
                    <Col xs={6}>
                          ...
                          <DropdownButton
                            ...
                            title={deployableFeeds.length
                              ? <span>
                                ...
                                {getMessage(messages, 'addFeedSource')}
                              </span>
                              : <span>{getMessage(messages, 'allFeedsAdded')}</span>
                            }
...
class CustomConfig extends Component {
  ...
  _onSaveConfig = () => {
    ...
    if (!this._isValidJSON(value)) return window.alert('Must provide valid JSON string.')
    else {
      updateDeployment(deployment, {[name]: value})
  ...
  render () {
    ...
    const validJSON = this._isValidJSON(value)
    return (
      <div>
