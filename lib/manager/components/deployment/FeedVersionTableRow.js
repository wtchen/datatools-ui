...
class FeedVersionTableRow extends Component<ConnectedRowProps> {
  _decrementVersion = () => {
...
const mapStateToProps = (state: AppState, ownProps: RowProps) => {
  ...
  const userCanDecrementVersion = userCanEditDeployment &&
    !feedVersionPinned &&
    version.previousVersionId

  const userCanIncrementVersion = userCanEditDeployment &&
    !feedVersionPinned &&
    version.nextVersionId

  // Cannot remove feed source if the deployment is feed-specific or it is
  // pinned
  const userCanRemoveVersion = userCanEditDeployment &&
    !feedVersionPinned &&
