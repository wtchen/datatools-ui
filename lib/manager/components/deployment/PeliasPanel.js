...
class PeliasPanel extends Component<Props, State> {
  ...
  */
  ...
  _onConfirmUpload = async (files: Array<File>) => {
    ...
    // Without the redundant await, flow is unhappy
    // eslint-disable-next-line no-return-await
    try {
      return await uploadPeliasWebhookCsvFile(deployment, file, fileToDeleteOnSuccesfulUpload)
    } catch (error) {
      const { setErrorMessage } = this.props
      const { message } = error
      // TODO: a prettier solution than doubled modals
      setErrorMessage({ title: 'Error while Uploading CSV File', message })
      return false
    }
  }
