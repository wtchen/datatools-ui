...
export default class FeedVersionSpanChart extends Component<Props> {
   ...
   * Note: Components below are all rendered as direct children of the parent element
   * (hence the returned array), so that the "today" marker appears
   * above the FeedSpan while remaining below the FeedStartLabel and FeedEndLabel contents.
  ...
  render () {
    ...
    const comparedVersion = comparedVersionProp || activeVersion
