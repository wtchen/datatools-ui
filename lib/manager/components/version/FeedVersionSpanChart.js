 ...
 * of the active and compared feed versions.
 */
export class FeedVersionSpanChartInner extends Component<ActiveOnlyProps> {
  render () {
    const { activeVersion } = this.props
    // Construct moment objects for the active/compared version start/end dates.
    const firstDate = moment(activeVersion.validationSummary.startDate)
    const lastDate = moment(activeVersion.validationSummary.endDate)
    ...
    const activeChartProps = {
      ...
      lastDate,
      top: 0,
      versionEndDate: lastDate,
      versionLabel: <VersionDateLabel version={activeVersion} />,
      versionStartDate: firstDate
    }

    return (
      <div style={{ paddingBottom: '20px', position: 'relative', width: '450px' }}>
        <FeedStartLabel {...activeChartProps} />
        <FeedEndLabel {...activeChartProps} />
        <FeedSpan {...activeChartProps} isServiceGap={false} />
        {showToday && <TodayMarker firstDate={firstDate} lastDate={lastDate} />}
