...
export default class FeedVersionNavigator extends Component<Props, State> {
  ...
  render () {
    ...
    const hasManyVersions = sortedVersions.length >= 2
    const comparedFeedVersion = sortedVersions[comparedVersionIndex - 1]
    const comparableVersions = sortedVersions.filter(v => v !== version)
            ...
            <ButtonToolbar>
              ...
              {this.state.listView
                ...
                : <ButtonGroup> {/* Version Navigation/Selection Widget */}
                  ...
                  {/* Show comparison version selector if 2+ versions present and comparing is active. */}
                  {hasManyVersions && comparedVersionIndex !== null &&
                    <VersionSelectorDropdown
                      dropdownProps={{
                        id: 'prevVersionSelector',
                        onSelect: this._onSelectComparedVersion
                      }}
                      header='Select a comparison version'
                      title={`Comparing to: ${this.messages('version')} ${comparedVersionIndex}`}
        ...
        <Row>
          <Col xs={12}>
            <FeedVersionViewer
              ...
              comparedVersion={comparedFeedVersion}
