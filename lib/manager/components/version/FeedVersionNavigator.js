...
export default class FeedVersionNavigator extends Component<Props, State> {
  ...
  /** Show comparison dropdown if there are 2+ versions to compare. */
  _renderComparisonDropdown = () => {
    const {
    ...
    if (sortedVersions.length >= 2) {
      ...
      const compareTitle = comparedFeedVersion ? `Comparing to Version ${comparedVersionIndex}` : 'Compare'

      // This custom formatter adds and hides the active version in the list to preserve version indexes.
      ...
      return (
        <VersionSelectorDropdown
          ...
          header='Select a version to compare'
            ...
            <ButtonToolbar>
              ...
              {this.state.listView
                ...
                : <ButtonGroup> {/* Version Navigation/Selection Widget */}
                  ...
                  {this._renderComparisonDropdown()}
                </ButtonGroup>
              }
