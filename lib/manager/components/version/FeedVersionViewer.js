...
import {errorPriorityLevels, formatDelta, getTableFatalExceptions} from '../../util/version'
...
class VersionSectionSelector extends Component<SelectorProps> {
  _renderIssuesLabel = () => {
    ...
    if (comparedVersion && comparedVersionIndex) {
      ...
      const diffClassName = diff > 0 ? 'label-danger' : (diff < 0 ? 'label-success' : 'label-default')
