...
class VersionSectionSelector extends Component<SelectorProps> {
  _renderIssuesLabel (
    ...
    if (comparedVersion) {
      ...
      const diffColor = diff > 0 ? 'label-danger' : (diff < 0 ? 'label-success' : 'label-default')
      const diffText = diff === 0 ? '=' : ((diff > 0 ? '+' : '') + numeral(diff).format('0a'))

      diffLabel = (
        <span className={diffColor} style={{border: '0.5px solid #ccc'}}>
          {diffText}
        </span>
      )
    }
  ...
  render () {
    const { version, comparedVersion } = this.props
    return (
      <Panel>
        <ListGroup fill>
          <LinkContainer
          ...
          <LinkContainer
            ...
            <ListGroupItem>
              <Icon type='exclamation-triangle' /> Validation issues {this._renderIssuesLabel(version, comparedVersion)}
            </ListGroupItem>
          </LinkContainer>
          ...
          <LinkContainer
            ...
            <ListGroupItem>
              ...
              <BsLabel htmlFor>{version.noteCount}</BsLabel>
            </ListGroupItem>
          </LinkContainer>
        </ListGroup>
      </Panel>
