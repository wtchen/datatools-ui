                ...
                return (
                  <Col xs={2} className='text-center' key={c}>
                    <StatisticWithVariation
                      showVariation={comparedVersion && comparedVersionIndex}
                      title={comparedVersionIndex && `Difference compared to Version ${comparedVersionIndex}`}
                      value={summary[c]}
                      variation={diff}
                    />
...
/**
 * Renders a statistic of a feed version with a small label attached to indicate a variation.
 */
const StatisticWithVariation = ({ showVariation, title, value, variation }) => {
  const bsStyle = variation > 0
    ? 'success'
    : (variation < 0 ? 'danger' : 'default')

  return (
    <span title={`${value}`} style={{ fontSize: '200%' }}>
      {numeral(value).format('0a')}
      {showVariation &&
        // Apply a zero-width to superscript so that `value` above always appears centered.
        <sup style={{ display: 'inline-block', width: 0 }}>
          <BsLabel
            style={{ fontSize: '50%', padding: '0 0.2em 0.1em' }}
            bsStyle={bsStyle}
            title={title}>
            {formatDelta(variation)}
          </BsLabel>
        </sup>}
    </span>
  )
