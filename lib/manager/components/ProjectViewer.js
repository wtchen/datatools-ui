...
type Props = {
  ...
  saveFeedSource: typeof feedsActions.saveFeedSource,
...
export default class ProjectViewer extends Component<Props, State> {
  ...
  deleteFeedSource (feedSource: Feed) {
    this.refs['page'].showConfirmModal({
      ...
      onConfirm: () => this.props.deleteFeedSource(feedSource)
    })
  }

  _onClickNewFeedSource = () => {
    this.props.createFeedSource(this.props.projectId)
  }
  ...
  componentWillMount () {
    this.props.onProjectViewerMount(this.props)
  }

  _onClickDeploy = () => this.props.deployPublic(this.props.project)

  _onSelectFilter = (key: string) => this.props.setVisibilityFilter(key)
