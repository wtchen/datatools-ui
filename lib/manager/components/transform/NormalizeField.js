...
type State = NormalizeFieldFields & {
  isPendingNewSubstitution: boolean,
  pendingDeleteIndex: number,
  pendingUpdateIndex: number
}
...
export default class NormalizeField extends Component<TransformProps<NormalizeFieldFields>, State> {
  // The 'pending' state vars below prevent a race condition when clicking the Add or Delete
  // substitution buttons after editing a field of an existing substitution.
  state = {
    isPendingNewSubstitution: false,
  ...
  componentDidUpdate (prevProps: TransformProps<NormalizeFieldFields>) {
    const { isPendingNewSubstitution, pendingDeleteIndex, pendingUpdateIndex } = this.state
    const prevSubstitutions = prevProps.transformation.substitutions
    const thisSubstitutions = this.props.transformation.substitutions
    if (pendingUpdateIndex !== -1 && prevSubstitutions && thisSubstitutions) {
      // If the substitution at the pending update index has been updated,
      // then clear the pending update index.
      if (!isEqual(prevSubstitutions[pendingUpdateIndex], thisSubstitutions[pendingUpdateIndex])) {
        this.setState({ pendingUpdateIndex: -1 })
      }
    } else {
      // If the substitution at the pending update index has been updated,
      // then proceed with any pending add/delete operation.
      if (isPendingNewSubstitution) {
        this._addNewSubstitution()
      } else if (pendingDeleteIndex !== -1) {
        this._removeSubstitution(pendingDeleteIndex)
      }
    }
  }
  ...
  _onChangeSubstitution = (substitution: Substitution, index: number) => {
    // Keep track of this index to let onBlur events complete
    // before adding or deleting substitutions.
    this.setState({ pendingUpdateIndex: index })
  ...
  _removeSubstitution = (index: number) => {
    // Clear pending flag before performing deletion
  ...
  _onRemoveSubstitution = (index: number) => {
    // Only mark the remove operation as pending
    // to allow any onBlur events on other fields to complete.
    this.setState({ pendingDeleteIndex: index })
  }

  _addNewSubstitution = () => {
    // Clear pending flag before performing addition
    this.setState({ isPendingNewSubstitution: false })
  ...
  _onNewSubstitution = () => {
    // Only mark the add operation as pending
    // to allow any onBlur events on other fields to complete.
    this.setState({ isPendingNewSubstitution: true })
  }
