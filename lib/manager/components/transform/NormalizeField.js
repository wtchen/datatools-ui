...
export default class NormalizeField extends Component<TransformProps<NormalizeFieldFields>, State> {
  ...
  _onNewSubstitution = () => {
    ...
    this.setState({ activeEditingIndex: (substitutions || []).length })
  }
  ...
  _getValidationErrors (fields: NormalizeFieldFields): Array<string> {
    ...
    if (substitutions && substitutions.filter(isSubstitutionInvalid).length > 0) {
  ...
  render () {
    ...
    const substitutions = originalSubstitutions || []
