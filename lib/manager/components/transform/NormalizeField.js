...
export default class NormalizeField extends Component<TransformProps<NormalizeFieldFields>, State> {
  ...
  /**
   * @returns The list of substitutions from backend or at least an empty list.
   */
  _getSubstitutions = () => this.props.transformation.substitutions || []
  ...
  _onNewSubstitution = () => {
    this.setState({ activeEditingIndex: this._getSubstitutions().length })
  }
  ...
  _getValidationErrors (fields: NormalizeFieldFields): Array<string> {
    ...
    if (this._getSubstitutions().filter(isSubstitutionInvalid).length > 0) {
  ...
  render () {
    ...
    const substitutions = this._getSubstitutions()
