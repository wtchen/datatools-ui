...
export default class NormalizeField extends Component<TransformProps<NormalizeFieldFields>, NormalizeFieldFields> {
  ...
  _onChangeFieldToNormalize = (fieldOption: ReactSelectOption) => {
    this._updateState({fieldName: fieldOption.value})
  }

  _onChangeCapitalize = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this._updateState({capitalize: evt.target.checked})
  }

  _onChangeCapitalizationExceptions = (values: Array<{value: string}>) => {
    this._updateState({capitalizationExceptions: values.map(v => v.value)})
  }

  _onChangePerformSubstitutions = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this._updateState({performSubstitutions: evt.target.checked})
  }
  ...
  _onChangeSubstitution = (substitution: Substitution, index: number) => {
    ...
    this._updateState({substitutions: newSubstitutions})
  }
  ...
  _onNewSubstitution = () => {
    this._updateState({substitutions: [...[].concat(this.state.substitutions), {
  ...
  _onRevertTransformationSettings = () => {
    this._updateState(extractStateFields(this.props.transformation))
  }
        ...
        <ul style={{ listStyleType: 'none', paddingLeft: '0px' }}>
          <li>
            ...
            <span style={{display: 'block'}}>
              <label htmlFor='capitalizationExceptionsList'>
              ...
              <FormGroup>
                <Select.Creatable
                  ...
                  clearable={false}
                  multi
                  id='capitalizationExceptionsList'
