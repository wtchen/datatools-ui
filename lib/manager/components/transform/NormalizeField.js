...
export default class NormalizeField extends Transform<NormalizeFieldFields> {
  ...
  _onChangeSubstitution = (substitution: Substitution, index: number) => {
    const newSubstitutions = [...this.state.substitutions]
  ...
  _onNewSubstitution = () => {
    this.updateState({substitutions: [...this.state.substitutions, {
 ...
 */
class SubstitutionRow extends Component<SubstitutionRowType> {
  ...
  _updateSubtitution (partialState: any) {
    const newSubstitution = {
      ...this.props.substitution,
      ...partialState
    }
    this.props.onChange(newSubstitution, this.props.index)
  }
