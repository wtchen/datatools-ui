...
export default class FeedTransformation extends Component<Props, {csvData: ?string, fieldName: ?string}> {
                ...
                <div>Exceptions: 
                  <FormGroup>
                    <FormControl
                      ...
                      type="text"
                      value={transformation.capitalizeExceptions ? transformation.capitalizeExceptions.join(',') : ''}
                    />
                  </FormGroup>                
                </div>
              </li>
  ...
  _onChangeCapitalizeExceptions = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this.props.onChange({capitalizeExceptions: evt.target.value.split(',')}, this.props.index)
  }
