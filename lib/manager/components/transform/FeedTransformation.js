...
export default class FeedTransformation extends Component<Props, {csvData: ?string, fieldName: ?string}> {
            ...
            <ul style={{ listStyleType: 'none', paddingLeft: '10px'}}>
              <li>
                <Checkbox
                  ...
                  onChange={this._onChangeCapitalizeField}
                >
                ...
                <div>Exceptions:
                  <FormGroup>
                    <FormControl
                      ...
                      type='text'
                      value={transformation.capitalizeExceptions || ''}
                    />
                  </FormGroup>
                </div>
              </li>
              <li>
                <Checkbox
                  checked={transformation.performSubstitutions}
                  inline
                ...
                <FormGroup>
                  <FormControl
                    ...
                    type='text'
                    value={transformation.substitutions || ''}
                  />
                </FormGroup>
              </li>
            </ul>
  ...
  _onChangeCapitalizeField = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this.props.onChange({capitalize: evt.target.checked}, this.props.index)
  }

  _onChangeCapitalizeExceptions = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this.props.onChange({capitalizeExceptions: evt.target.value}, this.props.index)
  }

  _onChangePerformSubstitutions = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this.props.onChange({performSubstitutions: evt.target.checked}, this.props.index)
  }

  _onChangeSubstitutions = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this.props.onChange({substitutions: evt.target.value}, this.props.index)
  }
