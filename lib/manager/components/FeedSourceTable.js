...
export default class FeedSourceTable extends MessageComponent<Props, State> {
  state = {
...
class FeedSourceTableRow extends MessageComponent<RowProps, RowState> {
  state = {
    hovered: false
  }
  ...
  _onChange = name => {
    const {feedSource, createFeedSource, project, updateFeedSource} = this.props
    if (feedSource.isCreating) createFeedSource({projectId: project.id, name})
...
class FeedSourceAttributes extends MessageComponent<{feedSource: Feed}> {
  render () {
...
class FeedSourceAttribute extends MessageComponent<AttributeProps> {
  render () {
