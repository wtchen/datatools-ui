...
/**
 * Get some UI data from either a ValidationResult or ValidationSummary
 */
function getVersionDisplayData (
  validationResult: ?(ValidationResult | ValidationSummary)
) {
  if (validationResult) {
    const errorCount = validationResult.errorCount || 'None'
    const endMoment = moment(validationResult.endDate)
    const startMoment = moment(validationResult.startDate)
...
export default class FeedSourceTableRow extends PureComponent<Props> {
  ...
  render () {
    ...
    const comparisonData = comparisonVersion &&
      getVersionDisplayData(comparisonVersion.validationResult)

    let comparisonSubtext, comparisonSubtextDate
    if (comparisonData && comparisonVersion) {
      if (comparisonData.status === 'future') {
        comparisonSubtext = `Starting in ${
          relativeDuration(comparisonVersion.validationResult.startDate)
        }`
        comparisonSubtextDate = comparisonVersion.validationResult.startDate
      } else if (comparisonData.status === 'expired') {
        comparisonSubtext = `Expired ${
          relativeDuration(comparisonVersion.validationResult.endDate)
        } ago`
        comparisonSubtextDate = comparisonVersion.validationResult.endDate
      } else {
    ...
    if (
      feedSource.latestVersionId &&
        comparisonVersion &&
        feedSource.latestVersionId === comparisonVersion.id
    ) {
