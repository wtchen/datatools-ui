...
import type {FeedSourceTableFilterStrategies, ManagerUserState} from '../../types/reducers'
  ...
  render () {
    ...
    return (
      <ListGroupItem key={feedSource.id} >
        ...
        />
        <Row>
          <FeedAttributes feedSource={feedSource} />
...
class FeedAttributes extends PureComponent<{ feedSource: Feed }> {
  render () {
...
class FeedVersionAttributes extends PureComponent<{
  ...
  render () {
    const {lastFetchedDate, retrievalMethod, validation} = this.props
