...
class DeploymentSettings extends Component {
  static propTypes = {
  ...
  _clearBuildConfig = () => {
    this.props.updateProjectSettings(this.props.project, {buildConfig: {}})
  }

  _clearRouterConfig = () => {
    this.props.updateProjectSettings(this.props.project, {routerConfig: {}})
  }
  ...
  _noEdits = () => Object.keys(this.state)
    .map(key => shallowEqual(this.state[key], this.props.project[key]))
    .indexOf(false) === -1
      ...
      <div key={project.lastUpdated} className='deployment-settings-panel'>
        ...
        <Panel header={
          <h4>
            ...
            <Icon type='cog' />{' '}
            {getMessage(messages, 'deployment.routerConfig.title')}
          </h4>
        }>
        ...
        <Panel header={
          <h4>
            <Button
              ...
              {getMessage(messages, 'deployment.routerConfig.updaters.new')}
            </Button>
            <Icon type='bolt' />{' '}
            {getMessage(messages, 'deployment.routerConfig.updaters.title')}
          </h4>
        }>
          {updaters.map((u, i) => (
            <CollapsiblePanel
              ...
              title={u.type
                ...
                : `[${getMessage(messages, 'deployment.routerConfig.updaters.placeholder')}]`
              }
            />
          ))}
        </Panel>
        ...
        <Panel header={<h4><Icon type='globe' /> {getMessage(messages, 'deployment.osm.title')}</h4>}>
          ...
          {project.useCustomOsmBounds || this.state.useCustomOsmBounds
            ? <FormGroup>
              ...
              <FormControl
                type='text'
                defaultValue={project.osmNorth !== null && typeof project.osmNorth !== 'undefined'
                  ? `${project.osmWest},${project.osmSouth},${project.osmEast},${project.osmNorth}`
                  : ''
                }
