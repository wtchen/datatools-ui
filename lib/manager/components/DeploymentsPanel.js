...
export default class DeploymentsPanel extends Component<Props> {
  ...
  _onToggleAutoDeploy = (evt: SyntheticInputEvent<HTMLInputElement>) => {
    this._updateProject({autoDeploy: evt.target.checked})
  }
  ...
  render () {
    ...
    const sortedDeployments = deployments
      ...
      : []
    const deployment = sortedDeployments.find(d => d.id && d.id === deploymentId)
    ...
    return (
      <Row>
        ...
        <Col sm={12} md={3}>
          <Panel header={
            <h3><Icon type='rocket' /> {this.messages('autoDeploy.title')}</h3>
          }>
            <FormGroup controlId='pinnedDeploymentId'>
              ...
              <Select
                ...
                options={sortedDeployments.map(d => ({label: d.name, value: d.id}))}
  ...
  render () {
    ...
    return (
      <tr className={this._isPinned() ? 'pinned-deployment' : ''}>
        ...
        <td>{server ? <BsLabel>{server.name}</BsLabel> : NA}</td>
