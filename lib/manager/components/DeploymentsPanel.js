...
export default class DeploymentsPanel extends Component<Props> {
  componentWillMount () {
    const {expanded, fetchProjectDeployments, project} = this.props
    if (expanded) fetchProjectDeployments(project.id)
  }
...
class DeploymentsList extends Component<ListProps, State> {
  ...
  render () {
    ...
    const pinnedDeployment = deployments &&
      pinnedDeploymentId &&
      deployments.find(deployment => deployment.id === pinnedDeploymentId)
    const visibleDeployments: Array<Deployment> = deployments
      ? deployments.filter(
        deployment =>
          ...
          deployment.id !== pinnedDeploymentId
      )
      : []
    return (
      <Panel
