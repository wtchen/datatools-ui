                ...
                <ListGroup key={index} fill>
                  ...
                  <Panel collapsible bsClass='' expanded={errorTypeIsActive}>
                    ...
                    <Table striped hover fill className='table-fixed'>
                      <thead><tr>
                        <th>Line #</th>
                        <th>{category.errors && category.errors[0] ? category.errors[0].entity_type : ''} ID</th>
                        <th>Bad value</th>
                        <th>Action</th>
                      </tr></thead>
                      {category.errors
                  ...
                  {errorTypeIsActive && category.errors.length < category.count
...
class ValidationErrorItem extends Component {
  ...
  _onClick = () => {
    ...
    if (!expanded) {
      if (!entity) {
        // fetch GTFS entity
        const {entity_id: id, entity_type: type} = error
        fetchGTFSEntities({type, id: [id], feedVersion})
      }
    }
  ...
  _onEditClick = () => console.log(this.props.error.entity_id)

  render () {
    const {error, gtfs, index} = this.props
    const {expanded} = this.state
    const {
      entity_id: id,
