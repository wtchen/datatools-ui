...
import {Alert, ListGroup, Table, ListGroupItem, Panel, Button, Col, Row} from 'react-bootstrap'
...
export default class GtfsValidationViewer extends Component {
  static propTypes = {
    fetchValidationIssueCount: PropTypes.func,
  ...
  componentWillMount () {
    this.props.fetchValidationIssueCount()
  }

  componentWillReceiveProps (nextProps) {
    if (
      this._validationHasErrors(this.props.validationResult) &&
      !this._validationHasErrors(nextProps.validationResult)
    ) {
  ...
  render () {
    ...
    const hasValidation = this._validationHasErrors(result)
