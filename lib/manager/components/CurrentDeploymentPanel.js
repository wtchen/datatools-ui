  ...
  render () {
    ...
    return (
      <Panel header={<h3><Icon type='server' /> Current deployment</h3>}>
        ...
        {deployJob || deployment.deployJobSummaries.length > 0
          ? <DeployJobSummary
...
class DeployJobSummary extends Component<{
  ...
  render () {
    const {deployJob, deployment, server, summary} = this.props
    const serverLabel = this._getLabelForServer(server, deployment)
    // If active deploy job is in progress, show current job status.
    if (deployJob) {
      return (
        <div>
          Deployment to {serverLabel} in progress...<br />
