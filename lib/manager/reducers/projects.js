...
const projects = (state: ProjectsState = defaultState, action: Action): ProjectsState => {
  switch (action.type) {
    ...
    case 'RECEIVE_FEEDSOURCE': {
      ...
      const sourceIndex = existingSources.findIndex(s => s.id === id)
    ...
    case 'PUBLISHED_FEEDVERSION': {
      const projectIndex = state.all.findIndex(p => p.id === action.payload.feedSource.projectId)
      const sourceIndex = state.all[projectIndex].feedSources.findIndex(s => s.id === action.payload.feedSource.id)
      const versionIndex = state.all[projectIndex].feedSources[sourceIndex].feedVersions.findIndex(v => v.id === action.payload.id)
      return update(state, {
    ...
    case 'RECEIVE_DEPLOYMENT': {
      const projectIndex = state.all.findIndex(p => p.id === action.payload.project.id)
