...
export const defaultState = {
  ...
  feedTableViewType: 'compact'
}

/* eslint-disable complexity */
const projects = (state: ProjectsState = defaultState, action: Action): ProjectsState => {
  switch (action.type) {
    case 'TOGGLE_FEED_TABLE_VIEW_TYPE':
      return update(
        state, {
          feedTableViewType: {
            $set: state.feedTableViewType === 'compact' ? 'detailed' : 'compact'
          }
        }
      )
    ...
    case 'RECEIVE_PROJECTS': {
      ...
      return update(state, {
        ...
        active: { $set: action.payload[activeIndex] && action.payload[activeIndex].id },
        filter: { $set: {
          filter: null,
          searchText: null
        }},
        sort: { $set: 'alphabetically-asc' }
      })
    }
