...
import { getAccountDisplayName } from '../../common/util/user'
import * as feedActions from '../../manager/actions/feeds'
import * as managerUserActions from '../../manager/actions/user'
import type { AccountType, Organization, Project, UserProfile } from '../../types'
...
export default class UserRow extends Component<Props, State> {
  ...
  render () {
    ...
    const { accountType, displayName, isDefault, isUnknown } = getAccountDisplayName(user, accountTypes)
    const displayedAccountType = !isUnknown
      ? displayName
      : 'Unknown'
        ...
        <tr>
          ...
          <td>
            {permissions.isApplicationAdmin()
              ? this._getUserPermissionLabel(permissions) // TODO: refactor app-admin label.
              : <BsLabel bsStyle={isUnknown ? 'warning' : isDefault ? 'default' : 'primary'}>{displayedAccountType}</BsLabel>
            }
          </td>
          <td>
          ...
          <td style={isEditing ? { display: 'none' } : null}>
