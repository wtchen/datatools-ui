...
export default class UserSettings extends React.Component<Props, State> {
  messages: {[string]: string} = getComponentMessages('UserSettings')
  ...
  getProjectLabel = (access: string): React.Node => {
    switch (access) {
      case 'none': return <Label>{getMessage(this.messages, 'project.noAccess')}</Label>
      case 'admin': return <Label bsStyle='primary'>{getMessage(this.messages, 'project.admin')}</Label>
      case 'custom': return <Label bsStyle='success'>{getMessage(this.messages, 'project.custom')}</Label>
      default: return null
    }
  }
      ...
      <Row>
          ...
          <Panel header={<h3>Organization settings</h3>}>
            {creatorIsApplicationAdmin &&
              <Checkbox
                ...
                {getMessage(this.messages, 'admin.title')}
              </Checkbox>
            }
              ...
              ? <div className='orgDetails'>
                ...
                {this.state.organization &&
                  <Checkbox
                    ...
                    {getMessage(this.messages, 'org.admin')}
                  </Checkbox>
                }
              </div>
              : null
        ...
        <Col xs={8}>
          {this.state.appAdminChecked
            ? <i>{getMessage(this.messages, 'admin.description')}</i>
            : this.state.orgAdminChecked
              ? <i>{getMessage(this.messages, 'org.description')}</i>
              : projectPanel
          }
        </Col>
      </Row>
