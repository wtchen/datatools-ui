...
1. Select the file or table to replace.
2. From the selector below the table selector, select the field to normalize.
3. "Enforce Title Case" will convert all words to Title Case regardless of how they appear in the original feed file.
4. You can enter/remove capitalization exceptions, especially for acronyms or prepositions. Some exceptions are already configured to appear by default. The exceptions will appear as entered.
5. You can enter substitutions that will replace a search term with a replacement term. There is one substitution per row in the substitutions table. Click the Edit ("pencil") button to modify a row. Click the Delete ("X") button to remove the row. Click "Add" to add a new substitution.
